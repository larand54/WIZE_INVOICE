...
type
  TfrmLoadArrivals = class(TForm)
    ...
    procedure acSendLoadToHampenExecute(Sender: TObject);
...
begin
 ...
 MailToAddress2 := '' ;
 MailToAddress  := '' ;
 if (dmArrivingLoads.cdsArrivingLoadsAVROP_CUSTOMERNO.AsInteger > 0) and (dmArrivingLoads.cdsArrivingLoadsAVROP_CUSTOMERNO.IsNull = False) then
 MailToAddress  := dmsContact.GetEmailAddress_Utlastad(dmArrivingLoads.cdsArrivingLoadsAVROP_CUSTOMERNO.AsInteger)
 else
 MailToAddress  := dmsContact.GetEmailAddress_Utlastad(dmArrivingLoads.cdsArrivingLoadsCUSTOMERNO.AsInteger) ;
 ...
 MailToAddress2 := dmsContact.GetEmailAddressForSpeditorByLO(dmArrivingLoads.cdsArrivingLoadsLO.AsInteger) ;
 ...
 Try
  SetLength(A, 1);
  A[0]  := dmArrivingLoads.cdsArrivingLoadsLOADNO.AsInteger ;//dmcOrder.cdsLoadsForLOLoadNo.AsInteger ;
  ...
  Begin
   Try
   dmsSystem.sq_PkgType_InvoiceByLO.ParamByName('LoadNo').AsInteger := dmArrivingLoads.cdsArrivingLoadsLOADNO.AsInteger ;// dmcOrder.cdsLoadsForLOLoadNo.AsInteger ;
   dmsSystem.sq_PkgType_InvoiceByLO.ExecSQL ;
     except
      On E: Exception do
      Begin
...
//      ShowMessage(E.Message);
   ...
   if dmsContact.Client_Language(dmArrivingLoads.cdsArrivingLoadsAVROP_CUSTOMERNO.AsInteger) = cSwedish then
 ...
 dmArrivingLoads.cdsArrivingLoadsLOADNO.AsString) ;//dmcOrder.cdsLoadsForLOLoadNo.AsString) ;
 ...
 End ;
 SetLength(Attach, 1);
 Attach[0]        := ExcelDir + 'FS ' + dmArrivingLoads.cdsArrivingLoadsLOADNO.AsString + '.pdf' ;
 dm_SendMapiMail  := Tdm_SendMapiMail.Create(nil);
 Try
  dm_SendMapiMail.SendMail('Följesedel. FSnr: ' + dmArrivingLoads.cdsArrivingLoadsLOADNO.AsString,
  'Följesedel bifogad. '
  +LF+''
  +'Load tally attached. '
  +LF+''
...
begin
 ...
 Try
  ...
  End ;
  dmVidaInvoice.ExportTypSoftSet(grdLoadsDBTableView1.DataController.DataSet.FieldByName('LoadNo').AsInteger,
  grdLoadsDBTableView1.DataController.DataSet.FieldByName('LO').AsInteger,
  4{RecordType},
  grdLoadsDBTableView1.DataController.DataSet.FieldByName('LOTYP').AsString, fExportLoadPurpose.rgPurpose.ItemIndex) ;
...
begin
 With dmArrivingLoads do
 Begin
  acSendLoadToHampen.Enabled := (cdsArrivingLoads.Active)
  and (cdsArrivingLoads.RecordCount > 0)
  and ((cdsArrivingLoads.FieldByName('OrderType').AsInteger = 1)
  or (cdsArrivingLoads.FieldByName('OrderType').AsInteger = -1)) ;
 End ;
...
begin
 acSamlingsFS_USA.Enabled:= grdLoadsDBTableView1.Controller.SelectedRecordCount > 0 ;
end;

procedure TfrmLoadArrivals.acPrintDirectFSUpdate(Sender: TObject);
begin
 acPrintDirectFS.Enabled:= grdLoadsDBTableView1.Controller.SelectedRecordCount > 0 ;
end;

procedure TfrmLoadArrivals.acPrintDirectFS_USAUpdate(Sender: TObject);
begin
 acPrintDirectFS_USA.Enabled:= grdLoadsDBTableView1.Controller.SelectedRecordCount > 0 ;
end;

procedure TfrmLoadArrivals.acPrintTallyUSNoteUpdate(Sender: TObject);
begin
 acPrintTallyUSNote.Enabled:= grdLoadsDBTableView1.Controller.SelectedRecordCount > 0 ;
end;
