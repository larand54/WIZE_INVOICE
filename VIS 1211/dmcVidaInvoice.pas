...
type
  ...
  private
    ...
    lbList              : TStringList ;
    function  GetAvdelning(const ClientNo : Integer;const KontoNr : String) : String ;
...
//    procedure Get500FilesFromFTP ;
  ...
  public
    ...
    procedure MailaCopyToVIDASTORE(const InvoiceNo : String;const InternalInvoiceNo, CustomerNo : Integer) ;
    ...
    procedure PrepareLindaExcelFile(const InvoiceNo : String;const CustomerNo, InternalInvoiceNo : Integer) ;
    Function  Check_DoesInvoiceValueCorrespondWith_Debit_Credit : String ;
    Function  GetInvoiceTypeFromBinding(const SalesRegionNo, OrderType : Integer) : Integer ;
    ...
    function  InsertVerifikatLogg (const Test : Boolean;const InvoiceNo, InternalInvoiceNo, CustomerNo, InvoiceType2 : Integer) : Boolean ;
...
Const FreeText  : String)  ;
    ...
    procedure PrepareExportFiles(const InvoiceNo : String;const CustomerNo, InternalInvoiceNo, CET, KV : Integer) ;
    ...
    function  CreateCreditInvoiceByCopyDebitInvoice(const InvoiceNo, InternalInvoiceNo : Integer;const DatumString : String;const InvoiceType : Integer) : Integer ;
...
Begin
 ...
 (cdsInvoiceHeadInvoiceType.AsInteger <> BORGSTENA_INVOICE))
 OR
...
//   FW_INVOICE                          : AssignFWInvoiceNumber(InternalInvoiceNo) ;
   ...
   VTA_INVOICE                          : AssignVTAInvoiceNumber(InternalInvoiceNo) ;
   BORGSTENA_INVOICE                    : Assign_BORGSTENA_InvoiceNumber(InternalInvoiceNo) ;
 End ;
End ;



procedure TdmVidaInvoice.Assign_BORGSTENA_InvoiceNumber(const InternalInvoiceNo : Integer) ;
Var mrResult  : Word ;
    InvoiceNo,
...
Begin
 Try
 sq_GetOrgInvoiceNoByCredit_BKO.ParamByName('NewInternalInvoiceNo').AsInteger := InternalInvoiceNo ;
...
Begin
 ...
 sq_InsBKOInvoiceNo.ParamByName('InternalInvoiceNo').AsInteger   := InternalInvoiceNo ;
 sq_InsBKOInvoiceNo.ParamByName('UserCreated').AsInteger         := ThisUser.UserID ;
 sq_InsBKOInvoiceNo.ParamByName('UserModified').AsInteger        := ThisUser.UserID ;
 sq_InsBKOInvoiceNo.ParamByName('DateCreated').AsSQLTimeStamp    := DateTimeToSQLTimeStamp(Now) ;
  ...
  Begin
   ...
   Begin
    ...
    //START A TRANSACTION
     ...
     Begin
      InsertInvoiceNoBKO(StrToInt(FormEnterInvoiceNo.eFakturanr.Text), InternalInvoiceNo) ;
...
procedure TdmVidaInvoice.AssignAGENTInvoiceNumber(const InternalInvoiceNo : Integer) ;
   ...
   Begin //Auto invoice number
    ...
    //START A TRANSACTION
     ...
     Begin
      ...
      if (cdsInvoiceHeadDebit_Credit.AsInteger = cCredit) and (cdsInvoiceHeadDelKredit.AsInteger = 0) and (OrgInternalInvoiceNo > 0) then
      Begin
       vis_CopyLoad( OrgInternalInvoiceNo, //Old OrgInternalInvoiceNo
       InternalInvoiceNo); //NewInternalInvoiceNo
       Insert_PkgType_Invoice ;
...
function TdmVidaInvoice.GetInvoiceNoOfMaxKeyTable(const InvoiceSerie : String) : Integer ;
Begin
...
procedure TdmVidaInvoice.AssignVTAInvoiceNumber(const InternalInvoiceNo : Integer) ;
Var mrResult  : Word ;
    InvoiceNo,
    OrgInternalInvoiceNo,
    QInvNo    : Integer ;
...
Begin
 ...
 sq_InsVTAInvNo.ParamByName('InternalInvoiceNo').AsInteger   := InternalInvoiceNo ;
 ...
 sq_InsVTAInvNo.ParamByName('DateCreated').AsSQLTimeStamp    := DateTimeToSQLTimeStamp(Now) ;
  ...
  Begin
   ...
   Begin
    ...
    //START A TRANSACTION
     ...
     if StrToInt(FormEnterInvoiceNo.eFakturanr.Text) > 0 then
     Begin
      InsertAgentInvoiceNo(StrToInt(FormEnterInvoiceNo.eFakturanr.Text), InternalInvoiceNo) ;
      if (cdsInvoiceHeadDebit_Credit.AsInteger = cCredit) and (cdsInvoiceHeadDelKredit.AsInteger = 0) then
      Begin
       vis_CopyLoad( OrgInternalInvoiceNo, //Old OrgInternalInvoiceNo
       InternalInvoiceNo); //NewInternalInvoiceNo
...
function TdmVidaInvoice.CreateCreditInvoiceByCopyDebitInvoice(const InvoiceNo, InternalInvoiceNo : Integer;const DatumString : String;const InvoiceType : Integer) : Integer ;
Var
...
//copy and insert InvoiceDetail
  ...
  if InvoiceType = 8 then
  Begin
   cdsInvoice_Credited_Komp.Active:= True ;
   cdsInvoice_Credited_Komp.Insert ;
   cdsInvoice_Credited_KompInternalInvoiceNo.AsInteger    := InternalInvoiceNo ;
   cdsInvoice_Credited_KompNewInternalInvoiceNo.AsInteger := cdsInvoiceHeadInternalInvoiceNo.AsInteger ;
   ...
   cdsInvoice_Credited_KompDateCreated.AsSQLTimeStamp     := DateTimeToSQLTimeStamp(Now) ;
  ...
  if cdsInvoice_Credited_Komp.ChangeCount > 0 then
   if cdsInvoice_Credited_Komp.ApplyUpdates(0) > 0 then myRollBack := True
    else
...
begin
 sq_GetInvoiceNumber.ParamByName('InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
 Try
...
procedure TdmVidaInvoice.PrepareExportFiles(const InvoiceNo : String;const CustomerNo, InternalInvoiceNo, CET, KV: Integer) ;
const
  LF = #10;
Var FileName,
    FileNameExcel,
...
Begin
 ...
 if ((SalesRegionNo = VIDA_WOOD_COMPANY_NO) or (SalesRegionNo = VIDA_BORGSTENA_BOTLR)) then
 Begin
  if CountryNo = Sweden then
   Result := Sweden
    else
...
// sp_xp_ldglogwrite.ParamByName('mop').AsString:= mop ; //varchar 8
 ...
 ShowMessage('#5: Serie = ' + Serie + '    Vouno = ' + inttostr(Vouno) + '    logno = ' + inttostr(logno)
 + '  cid = ' + cid) ;
...
// ShowMessage('procedure TdmVidaInvoice.EXEC_sp_VIS_xp_voulogwrite');
 ...
 if (thisuser.UserName = 'larmak') or (thisuser.UserName = 'Lars') then
 ShowMessage('#1 EXEC_sp_VIS_xp_voulogwrite: Serie = ' + Serie + '    Vouno = ' + inttostr(Vouno) + '    logno = ' + inttostr(logno)) ;
 ...
 if (thisuser.UserName = 'larmak') or (thisuser.UserName = 'Lars') then
 ShowMessage('#2 EXEC_sp_VIS_xp_voulogwrite: Serie = ' + Serie + '    Vouno = ' + inttostr(Vouno) + '    logno = ' + inttostr(logno)) ;
...
Begin
 ...
 if (thisuser.UserName = 'larmak') or (thisuser.UserName = 'Lars') then
 ShowMessage('EXEC_xp_vourowlogwrite: ' + LF +
 ' logno = ' + IntToStr(logno) + LF +
 ' account = ' + account  +   LF +
 ' currency = ' + currency +LF +
 ' amount = ' + FloatToStr(amount)+LF +
 ' Object2 = ' + Object2+LF +
 ' Object5 = ' + Object5+LF +
 ' FreeText = ' + FreeText) ;
...
function TdmVidaInvoice.InsertVerifikatLogg (const Test : Boolean;const InvoiceNo, InternalInvoiceNo, CustomerNo, InvoiceType2 : Integer) : Boolean ;
const
  LF = #10;
...
//   TotalInvoiceValueNoRounding_SEK  := sp_InvTotalsTotalInvoiceValueNoRounding_SEK.AsFloat ;
   InvoiceTotal_SEK                 := sp_InvTotalsInvoiceTotalSEK.AsFloat ;//FAKTURATOTAL I SEK
//   InvVal_No_moms_freight_SEK       := sp_InvTotalsInvValnofreight_SEK.AsFloat ;//FAKTURA BELOPP SEK UTAN moms_sek OCH FRAKT
   InvoiceTotal_For                 := sp_InvTotalsInvoiceTotalFOR.AsFloat ;//FAKTURATOTAL I UTLÄNDSVALUTA
...
//   VaruGrupp      := sp_InvTotalsVaruGrupp.AsInteger ;
   ...
   SkatteUpplag   := sp_InvTotalsSkatteUpplag.AsInteger ;//0 = INGET SKATTEUPPLAG, 1 = SKATTEUPPLAG
...
//RAD 1 KUNDFORDRING
   ...
   mt_VerLoggKurs.AsFloat           := Abs(sp_InvTotalsInvoiceTotalSEK.AsFloat) / Abs(sp_InvTotalsInvoiceTotalFOR.AsFloat)
   else
...
//RAD 4 MOMS
   ...
   mt_VerLoggKurs.AsFloat           := sp_InvTotalsVAT_SEK.AsFloat / sp_InvTotalsVAT_FOR.AsFloat
   else
   mt_VerLoggKurs.AsFloat           := 0 ;
...
//RAD 2 VARA
   ...
   Begin
    ...
    mt_VerLoggKurs.AsFloat           := sp_InvTotalsProduktvardeSEK.AsFloat / sp_InvTotalsProduktvardeFOR.AsFloat
    else
    mt_VerLoggKurs.AsFloat           := 0 ;
...
//      mt_VerLoggUtlandsBelopp.AsFloat  := sp_InvTotalsTotal_Product_Value_No_Freight_For.AsFloat* -1 ; //  sp_InvTotalsProduktvardeFOR.AsFloat * -1 ;
      ...
      mt_VerLoggKredit.AsFloat         := int(sp_InvTotalsProduktvardeSEK.AsFloat * -1 * 10) ;
      mt_VerLoggKredit.AsFloat         := mt_VerLoggKredit.AsFloat / 10 ;
      mt_VerLoggUtlandsBelopp.AsFloat  := int(sp_InvTotalsProduktvardeFOR.AsFloat * -1 * 10) ;
      mt_VerLoggUtlandsBelopp.AsFloat  := mt_VerLoggUtlandsBelopp.AsFloat / 10 ;
     End
     else
     Begin
      mt_VerLoggKredit.AsFloat         := 0 ;
...
//      mt_VerLoggUtlandsBelopp.AsFloat  := sp_InvTotalsTotal_Product_Value_No_Freight_For.AsFloat* -1 ; // sp_InvTotalsProduktvardeFOR.AsFloat * -1 ;
      mt_VerLoggDebet.AsFloat          := int(sp_InvTotalsProduktvardeSEK.AsFloat * -1 * 10) ;
...
//     if (sp_InvTotalsProduktvardeSEK.AsFloat < 0) or (sp_InvTotalsInclInPrice.AsInteger = 0) then
     ...
     Begin
      mt_VerLoggKredit.AsFloat         := Abs(sp_InvTotalsProduktvardeSEK.AsFloat) * -1 ;
      mt_VerLoggDebet.AsFloat          := 0 ;
      mt_VerLoggUtlandsBelopp.AsFloat  := Abs(sp_InvTotalsProduktvardeFOR.AsFloat) * -1 ;
     End
     else //Kredit
     Begin
      mt_VerLoggKredit.AsFloat         := 0 ;
      mt_VerLoggDebet.AsFloat          := sp_InvTotalsProduktvardeSEK.AsFloat ;// * -1 ;
      mt_VerLoggUtlandsBelopp.AsFloat  := sp_InvTotalsProduktvardeFOR.AsFloat ;
     End ;
    End ;
 ...
 //   if SalesRegionNo = VIDA_BORGSTENA_BOTLR then
     mt_VerLoggAvdelning.AsString := GetAvdelning(SalesRegionNo, mt_VerLoggKonto.AsString) ;
//     else
      ...
      ResKontraSerie:= 'VWK2'
        ...
        if InvoiceType2 = 5 then
         ResKontraSerie:= 'VWK4'
           ...
           if InvoiceType2 = 7 then
            ResKontraSerie:= 'VTA1'
             else
              if InvoiceType2 = 8 then
              ResKontraSerie:= 'VBAB' ;
...
//RAD 5 Öresavrundning
   ...
   if (mt_VerLoggDebet.AsFloat > 10) or (Abs(mt_VerLoggKredit.AsFloat) > 10) then
    Result  := False
     else
      Result  := True ;
...
//*******************************************
    ...
    Begin
      ...
      Begin
        InvoiceTotal_SEK  := mt_VerLoggDebet.AsFloat ;//         := sp_InvTotalsInvoiceTotalSEK.AsFloat ;
      End
      else
      Begin
        InvoiceTotal_SEK  := mt_VerLoggKredit.AsFloat ;//        := sp_InvTotalsInvoiceTotalSEK.AsFloat ;
      End ;

      if (thisuser.UserName = 'larmak') or (thisuser.UserName = 'Lars') then
      ShowMessage('mt_VerLoggAvdelning.AsString = ' + mt_VerLoggAvdelning.AsString);
      ...
      'mt_VerLoggKonto.AsString(KundResKontra) = ' + mt_VerLoggKonto.AsString +LF+
      'InvoiceTotal_SEK = ' + Floattostr(InvoiceTotal_SEK) +LF+
      'Object2 = ' + Object2+LF+
      'mt_VerLoggUtlandsBelopp.AsString = ' + mt_VerLoggUtlandsBelopp.AsString) ;
...
//*******************************************
      ...
      mt_VerLoggKonto.AsString,//KundResKontra account tex 1560
      InvoiceDate, //logdate
      mt_VerLoggValuta.AsString, //currency
      InvoiceTotal_SEK,
      mt_VerLoggUtlandsBelopp.AsFloat, // InvoiceTotal_For,
...
//*******************************************
    ...
    Begin
     ...
     if (thisuser.UserName = 'larmak') or (thisuser.UserName = 'Lars') then
      ShowMessage('(RadOrder > 1) Skapavaruvärde verifikationsrad: ' +LF+
      'mt_VerLoggKonto.AsString(KundResKontra) = ' + mt_VerLoggKonto.AsString +LF+
      'ForsaljningsKonto_SEK = ' + Floattostr(ForsaljningsKonto_SEK)+LF+
      'InvoiceTotal_SEK = ' + Floattostr(InvoiceTotal_SEK) +LF+
      'Object2 = ' + Object2+LF+
      'Object5 = ' + Object5+LF+
      'mt_VerLoggUtlandsBelopp.AsString = ' + mt_VerLoggUtlandsBelopp.AsString+LF+
      'mt_VerLoggRadOrder =  ' + mt_VerLoggRadOrder.AsString+LF+
      'Length(TRIM(mt_VerLoggKonto.AsString)) = ' + IntToStr(Length(TRIM(mt_VerLoggKonto.AsString)))) ;
...
//      if ForsaljningsKonto_SEK <> 0 then
      if (Length(TRIM(mt_VerLoggKonto.AsString)) > 0)
      and (ForsaljningsKonto_SEK <> 0)  then
      EXEC_xp_vourowlogwrite (logno,
      mt_VerLoggKonto.AsString,//account tex 3020
      InvoiceDate, //logdate
      mt_VerLoggValuta.AsString, //currency
//   InvoiceTotal_SEK,
//   InvoiceTotal_For,
//Tog bort *-1 på nedan 2 rader
      ...
      mt_VerLoggUtlandsBelopp.AsFloat, //ForsaljningsKonto_For, //foramount : double;
...
Function TdmVidaInvoice.GetInvoiceTypeFromBinding(const SalesRegionNo, OrderType : Integer) : Integer ;
Begin
...
Function TdmVidaInvoice.Check_DoesInvoiceValueCorrespondWith_Debit_Credit : String ;
Begin
 if (cdsInvoiceHeadInv_Value_To_Be_Paid_2.AsFloat > 0) and (cdsInvoiceHeadDebit_Credit.AsInteger = 0) then
 Begin
  Result  := '' ;
 End
 else
 if (cdsInvoiceHeadInv_Value_To_Be_Paid_2.AsFloat < 0) and (cdsInvoiceHeadDebit_Credit.AsInteger = 1) then
 Begin
  Result  := '' ;
 End
 else
...
procedure TdmVidaInvoice.PrepareLindaExcelFile(const InvoiceNo : String;const CustomerNo, InternalInvoiceNo : Integer) ;
const
  LF = #10;
Var //FileName,
    FileNameExcel,
...
//  FileNameExcel  := 'C:\vis\InvoiceNo ' + InvoiceNo + '.xls' ;
 ...
 cds_LindaExport.ParamByName('InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
...
begin
   ...
   Begin
     Screen.Cursor := crSQLWait;    { Show hourglass cursor }
     sp_vis_DelPrelInvoice.ParamByName('@InternalInvoiceNo').AsInteger  := InternalInvoiceNo ;
...
//Var myYear, myMonth, myDay,
 Var NoOfDaysDiscount2 : Word ;
Begin
 NoOfDaysDiscount2  := 0 ;
// DecodeDate(cdsInvoiceHeadInvoiceDate.AsDateTime, myYear, myMonth, myDay);

 NoOfDaysDiscount2  := dmsSystem.Get_NoOfDaysDiscount2(cdsInvoiceHeadPaymentTermsNo.AsInteger) ;

 if NoOfDaysDiscount2 > 0 then
 Begin
  if cdsInvoiceHead.State in [dsBrowse] then
   cdsInvoiceHead.Edit ;
  cdsInvoiceHeadDueDate.AsDateTime := IncDay(cdsInvoiceHeadInvoiceDate.AsDateTime, NoOfDaysDiscount2);
 End ;
End;
...
function TdmVidaInvoice.CustomerHaveValidEAN(const InternalInvoiceNo : Integer) : Boolean ;
Var ResultText : String ;
Begin
 ResultText := '' ;
 sp_CustomerHaveValidEAN.ParamByName('@InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
...
function TdmVidaInvoice.GetAvdelning(const ClientNo : Integer;const KontoNr : String) : String ;
Var ResultText : String ;
Begin
...
begin
 ...
 Begin
   ...
   Begin
    Try
    cds_InsertToInvoiceEDI.ParamByName('InternalInvoiceNo').AsInteger   := InternalInvoiceNo ;
    cds_InsertToInvoiceEDI.ParamByName('UserID').AsInteger              := ThisUser.UserID ;
    cds_InsertToInvoiceEDI.ParamByName('DateCreated').AsSQLTimeStamp    := DateTimeToSQLTimeStamp(Now) ;
...
Begin
 sp_vis_DeleteInvoicedEDI.ParamByName('@InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
...
function TdmVidaInvoice.InvoiceSentAsEDI(const InternalInvoiceNo : Integer) : Boolean ;
Begin
 sp_vis_InvoicedEDIStatus.ParamByName('@InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
 ...
 if Result then
 Begin
  if MessageDlg('The invoice has already been sent by EDI, do you want to do a resend?',
  mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  Begin
...
procedure TdmVidaInvoice.MailaCopyToVIDASTORE(const InvoiceNo : String;const InternalInvoiceNo, CustomerNo : Integer) ;
const
  LF = #10;
Var FormCRExportOneReport   : TFormCRExportOneReport ;
    A                       : array of variant ;
...
// if dmVidaInvoice.cdsInvoiceHeadInternalInvoiceNo.AsInteger < 1 then exit ;
 ...
 Try
  ...
  FormCRExportOneReport.CreateCo(CustomerNo, cFaktura, A, ExcelFolder + 'InvoiceNo ' + InvoiceNo) ;
...
// Attach[1]:= ExtractFilePath(Forms.Application.ExeName) + '\'+'Specification '+InvoiceNo+'.pdf' ;
 dm_SendMapiMail         := Tdm_SendMapiMail.Create(nil);
 Try
  dm_SendMapiMail.SendMail('Fakturanr: '+InvoiceNo
  +'InvoiceNo: '+InvoiceNo,
  'Faktura bifogad. '
  +LF+''
  +'Invoice attached. '
...
procedure TdmVidaInvoice.PkgLogInvoiced (const InternalInvoiceNo, Operation : Integer) ;
Begin
 Try
 sp_PkgLogInvoiced.ParamByName('@InternalInvoiceNo').AsInteger  :=  InternalInvoiceNo ;
...
Begin
 Try
  sp_DEL_ExportInvoiceData.ParamByName('@UserID').AsInteger  := ThisUser.UserID ;
...
procedure TdmVidaInvoice.STORE_ExportInvoiceData(mtSelectedInvoices : TkbmMemTable) ;
Begin
 mtSelectedInvoices.First ;
 while not mtSelectedInvoices.Eof do
 Begin
  Try
  sp_STORE_ExportInvoiceData.ParamByName('@IntInvoiceNo').AsInteger   := mtSelectedInvoices.FieldByName('InternalInvoiceNo').AsInteger ;
  sp_STORE_ExportInvoiceData.ParamByName('@UserID').AsInteger         := ThisUser.UserID ;
