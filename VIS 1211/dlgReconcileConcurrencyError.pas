...
unit dlgReconcileConcurrencyError;
...
const
  ActionStr : array [TReconcileAction] of string = ('Skip', 'Abort',
    'Merge', 'Correct', 'Cancel', 'Refresh' );
  UpdateKindStr : array [TUpdateKind] of string = ('Modified',
    'Inserted', 'Deleted' );
...
type
  TReconcileErrorForm = class(TForm)
    ...
    procedure UpdateDataSetEditText(Sender: TObject; ACol, ARow: integer; const
                                    Value: String);
    procedure DisplayFieldValues(Sender: TObject);
    procedure UpdateDataSelectCell(Sender: TObject; Col, Row: integer; var
                                   CanSelect: Boolean);
  private
    FDataSet: TDataset;
  ...
  public
    constructor CreateForm(DataSet: TDataset; UpdateKind: TUpdateKind; Error:
                           EReconcileError);
  end;
...
const
  BinaryDataTypes : set of TFieldType = [ftBytes, ftVarBytes, ftBlob, ftGraphic
  ..ftCursor];
begin
  try
    if VarIsClear(V) then
      ...
      if DataType in BinaryDataTypes then
        ...
        if DataType in [ftAdt] then
          Result := SAdt
        else
          if DataType in [ftArray] then
            Result := SArray
          else
            Result := VarToStr(V)
  except
...
{ TReconcileErrorForm }
constructor TReconcileErrorForm.CreateForm(DataSet: TDataset; UpdateKind: TUpdateKind;
...
begin
  ...
  if FUpdateKind <> ukDelete then begin
  ...
  if HasCurValues then begin
    FCurColIdx := FColCount;
    Inc(FColCount);
    UpdateData.Cells[FCurColIdx, 0] := SConflict
    end;

  if FUpdateKind = ukModify then begin
    FOldColIdx := FColCount;
    Inc(FColCount);
    UpdateData.Cells[FOldColIdx, 0] := SOriginal
    end;
...
begin
  ...
  for i := 0 to FDataFields.Count - 1 do
    with PFieldData(FDataFields[i])^ do begin
      ...
      if FNewColIdx > 0 then begin
        case Action of
          raCancel ,
          raRefresh: UpdateData.Cells[FNewColIdx, CurRow] := SUnchanged;
          raCorrect: if Edited then
                       UpdateData.Cells[FNewColIdx, CurRow] := EditValue
                     else
                       UpdateData.Cells[FNewColIdx, CurRow] := NewValue;
          else       UpdateData.Cells[FNewColIdx, CurRow] := NewValue
          end;
...
var
  ...
  i: integer;
begin
  with UpdateData do begin
    NewWidth := (ClientWidth - ColWidths[0]) div (ColCount -
                1);
...
begin
  ...
  UpdateData.DefaultRowHeight := UpdateData.Canvas.TextHeight(
                      'SWgjp') + 7 { Do not localize }
end;
...
procedure TReconcileErrorForm.UpdateDataSetEditText(
  Sender: TObject;
  ACol, ARow: integer;
  const Value: String);
begin
...
procedure TReconcileErrorForm.UpdateDataSelectCell(Sender: TObject; Col, Row: integer;
