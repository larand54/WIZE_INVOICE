...
uses
  ...
  ImgList, FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param,
  FireDAC.Stan.Error,
...
type
  ...
  private
    ...
    function GetPackageTypeNo(Const PKGCODE: String; Var PCSPERPKG: Integer;
      Var sActualLengthMM: String): Integer;
    { Private declarations }
  public
    // PcsPerLengthArray, byt ut mot AVERAGE_LengthArray om inputoption är 2
    PkgNoSeries, CopyPcs, ChangeNoOfPcsEvent: Boolean;
    CSD: Byte;
    PcsPerLengthArray: Array [0 .. 400] of Integer;
    // to copy no of pcs to new row
    ...
    function PackageNumberExists(const PkgNo: Integer;
    ...
    function SaveImportedPackages(const SaveToProdLogg, ProducerNo,
      LogicalInventoryPointNo, RegPointNo, RunNo: Integer;
    ...
    function SaveArticlePackages(const SaveToProdLogg, ProducerNo,
      LogicalInventoryPointNo, RegPointNo, RunNo: Integer;
      const WhenCreated: TDateTime): Boolean;
  end;

var
  dmsProduct: TdmsProduct;
...
begin
  ...
  begin
    ...
    mtStandardLengthsProductLengthNo.AsInteger :=
      sp_StandardLengthsProductLengthNo.AsInteger;
...
function TdmsProduct.SaveImportedPackages(const SaveToProdLogg, ProducerNo,
  LogicalInventoryPointNo, RegPointNo, RunNo: Integer;
  const WhenCreated: TDateTime): Boolean;
Var
  Save_Cursor: TCursor;
...
var
  ...
  Var
    ...
    Begin
      Result := 0;
      mtImportedProductLengths.Filter := 'PackageNo = ' + mtPackages.Fields
        [cPACKAGENO].AsString;
      ...
      Try
        ...
        Begin
          New(ARecord);
          ARecord^.ProductLengthNo := dmsSystem.GetProductLengthNoByLengthRule
            (mtImportedProductLengthsALMM.AsString,
    ...
    Begin
      ...
      Begin
        ...
        Begin
          New(ARecord);
          ARecord^.ProductLengthNo := dmsSystem.GetProductLengthNoByLengthRule
            (mtPackages.Fields[x].DisplayLabel,
            mtPackages.FieldByName('NOM_THICK').AsFloat,
            mtPackages.FieldByName('SurfacingNo').AsInteger);
    ...
    Begin
      ...
      Result := 0;
      New(ARecord);
      ARecord^.ProductLengthNo := dmsSystem.GetProductLengthNoByALMM
        (FloatToStr(ML));
    ...
    Var
      ...
      ML: Double;
    Begin
      ML := (mtPackages.Fields[FirstLengthColumnInmatningViaLast]
        .AsFloat * 1000);
      Result := 0;
      New(ARecord);
      ARecord^.ProductLengthNo := dmsSystem.GetProductLengthNoByALMM
  ...
  Begin
    ...
    else if Self.InputOption = INPUT_PIECES then
      GetNoOfLengthsInPkg
    else if Self.InputOption = INPUT_LOPM_TOTAL_LOPM then
      GetNoOfLengthsInPkg_LPM_Entry
    else if Self.InputOption = INPUT_LOPM_AVERAGE_LENGTH then
      GetNoOfLengthsInPkg_LPM_Entry_AVERAGE_LENGTH_OPTION;
  ...
  Var
    ...
    begin
      ...
      sp_PackageTypes.ParamByName('@ProductNo').AsInteger :=
      ...
      if mtPackages.Fields[cBARCODEID].AsInteger = 0 then
        ...
        sp_PackageTypes.ParamByName('@BarCodeID').AsInteger :=
      ...
      if mtPackages.Fields[cGRADESTAMP].AsInteger = 0 then
        ...
        sp_PackageTypes.ParamByName('@GradeStamp').AsInteger :=
          mtPackages.Fields[cGRADESTAMP].AsInteger;
      sp_PackageTypes.ParamByName('@TotalPieces').AsInteger :=
        mtPackages.Fields[cTOTALPCS].AsInteger;

      // if (mtpackages.Active) AND (mtpackages.Fields[1].AsInteger > 0) then
      sp_PackageTypes.ParamByName('@PcsPerLength').AsString :=
        GetLengthsEntered;
      // else
      ...
      try
        ...
        if not sp_PackageTypes.Eof then
        begin
    ...
    begin
      ...
      Try
        ...
        sp_NewPackageType.ParamByName('@PackageTypeNo').AsInteger :=
          PackagetypeNo;
        sp_NewPackageType.ParamByName('@ProductNo').AsInteger :=
          mtPackages.Fields[cPRODUCTNO].AsInteger;

        if mtPackages.Fields[cBARCODEID].AsInteger = 0 then
          sp_NewPackageType.ParamByName('@BarCodeID').AsInteger := 0
        else
          sp_NewPackageType.ParamByName('@BarCodeID').AsInteger :=
            mtPackages.Fields[cBARCODEID].AsInteger;

        if mtPackages.Fields[cGRADESTAMP].AsInteger = 0 then
          sp_NewPackageType.ParamByName('@GradeStamp').AsInteger := 0
        else
          sp_NewPackageType.ParamByName('@GradeStamp').AsInteger :=
            mtPackages.Fields[cGRADESTAMP].AsInteger;
        sp_NewPackageType.ParamByName('@TotalNoOfPieces').AsInteger :=
          mtPackages.Fields[cTOTALPCS].AsInteger;
      ...
      begin
        ARecord := MyList.Items[iDtl];
        Try
          sp_NewPackageDetail.ParamByName('@PackageTypeNo').AsInteger :=
            PackagetypeNo;
          sp_NewPackageDetail.ParamByName('@ProductLengthNo').AsInteger :=
            ARecord^.ProductLengthNo;
          sp_NewPackageDetail.ParamByName('@NoOfPieces').AsInteger :=
            ARecord^.NoOfPieces;
          sp_NewPackageDetail.ParamByName('@UserID').AsInteger :=
            ThisUser.UserID;
    ...
    Begin
      Try
        ...
        sp_Populate_One_PkgTypeLengths.ParamByName('@SearchPackageTypeNo')
          .AsInteger := PackagetypeNo;
  ...
  begin
    ...
    Begin
      Try
        sp_NewPackageNo.ParamByName('@PackageNo').AsInteger :=
          mtPackages.Fields[cPACKAGENO].AsInteger;
        sp_NewPackageNo.ParamByName('@PackageTypeNo').AsInteger :=
          PackagetypeNo;
        sp_NewPackageNo.ParamByName('@SupplierCode').AsString :=
          Trim(mtPackages.Fields[cSUPPLIERCODE].AsString);
        sp_NewPackageNo.ParamByName('@LogicalInventoryPointNo').AsInteger :=
          LogicalInventoryPointNo;
        sp_NewPackageNo.ParamByName('@SupplierNo').AsInteger := SupplierNo;
        sp_NewPackageNo.ParamByName('@RegistrationPointNo').AsInteger :=
          RegPointNo;
        sp_NewPackageNo.ParamByName('@UserID').AsInteger := ThisUser.UserID;
        sp_NewPackageNo.ParamByName('@CreatedOfPkgStr').AsInteger := 0;
        // 0 = Manual entry
        sp_NewPackageNo.ParamByName('@DateCreated').AsSQLTimeStamp :=
          DateTimeToSQLTimeStamp(WhenCreated);
    ...
    Begin
      Try
        sp_NewPkgNoNoProdLogg.ParamByName('@PackageNo').AsInteger :=
          mtPackages.Fields[cPACKAGENO].AsInteger;
        sp_NewPkgNoNoProdLogg.ParamByName('@PackageTypeNo').AsInteger :=
          PackagetypeNo;
        sp_NewPkgNoNoProdLogg.ParamByName('@SupplierCode').AsString :=
          Trim(mtPackages.Fields[cSUPPLIERCODE].AsString);
        sp_NewPkgNoNoProdLogg.ParamByName('@LogicalInventoryPointNo').AsInteger
          := LogicalInventoryPointNo;
        sp_NewPkgNoNoProdLogg.ParamByName('@SupplierNo').AsInteger :=
          SupplierNo;
        sp_NewPkgNoNoProdLogg.ParamByName('@RegistrationPointNo').AsInteger :=
          RegPointNo;
        sp_NewPkgNoNoProdLogg.ParamByName('@UserID').AsInteger :=
          ThisUser.UserID;
        sp_NewPkgNoNoProdLogg.ParamByName('@CreatedOfPkgStr').AsInteger := 0;
        // 0 = Manual entry
        sp_NewPkgNoNoProdLogg.ParamByName('@DateCreated').AsSQLTimeStamp :=
          DateTimeToSQLTimeStamp(WhenCreated);
    ...
    Begin
      ...
      try
        ...
        Try
          ...
          ShowMessage('Could not save Package no ' +
            Pkg_Info(mtPackages.Fields[cPACKAGENO].AsInteger,
            Trim(mtPackages.Fields[cSUPPLIERCODE].AsString)));
          Result := False;
          Raise;
        End;
        ...
        mtDupPkgNo.Insert;
        mtDupPkgNoPackageNo.AsInteger := mtPackages.Fields[cPACKAGENO]
          .AsInteger;
        mtDupPkgNoSupplierCode.AsString :=
          Trim(mtPackages.Fields[cSUPPLIERCODE].AsString);
        mtDupPkgNo.Post;
...
function TdmsProduct.SaveImportedWoodXPackages
  (mtUserProp: TkbmMemTable): Boolean;
Var
  Save_Cursor: TCursor;
const
  PKGS_CREATED_AT_PLANER_MILL = 1;
var
            ...
            Begin
              Try
                sp_NewPackageDetail.ParamByName('@PackageTypeNo').AsInteger :=
                  PackagetypeNo;
                sp_NewPackageDetail.ParamByName('@ProductLengthNo').AsInteger :=
                  cds_ImpLenMapProductLengthNo.AsInteger;
                sp_NewPackageDetail.ParamByName('@NoOfPieces').AsInteger :=
                  cds_ImpLenMapStyck.AsInteger;
                sp_NewPackageDetail.ParamByName('@UserID').AsInteger :=
                  ThisUser.UserID;
                sp_NewPackageDetail.ExecProc;
              except
                ...
                Begin
                  dmsSystem.FDoLog
                    (E.Message + ' :sp_NewPackageDetail.ExecProc');
                  Raise;
                End;
              end;
              cds_ImpLenMap.Next;
  ...
  begin
    ...
    Begin
      Try
        sp_NewPkgNo_Price_NoPP.ParamByName('@PackageNo').AsInteger :=
          cds_ImpProdMapIdentifier.AsInteger;
        sp_NewPkgNo_Price_NoPP.ParamByName('@PackageTypeNo').AsInteger :=
          PackagetypeNo;
        sp_NewPkgNo_Price_NoPP.ParamByName('@SupplierCode').AsString :=
          cds_ImpProdMapClientCode.AsString;
        sp_NewPkgNo_Price_NoPP.ParamByName('@LogicalInventoryPointNo').AsInteger
          := mtUserProp.FieldByName('LIPNo').AsInteger;
        sp_NewPkgNo_Price_NoPP.ParamByName('@SupplierNo').AsInteger :=
          mtUserProp.FieldByName('OwnerNo').AsInteger;
        sp_NewPkgNo_Price_NoPP.ParamByName('@RegistrationPointNo').AsInteger :=
          20; // Inköp //mtUserProp.FieldByName('RegPointNo').AsInteger ;
        sp_NewPkgNo_Price_NoPP.ParamByName('@UserID').AsInteger :=
          ThisUser.UserID;
        sp_NewPkgNo_Price_NoPP.ParamByName('@CreatedOfPkgStr').AsInteger := 0;
        // 0 = Manual entry
        sp_NewPkgNo_Price_NoPP.ParamByName('@DateCreated').AsSQLTimeStamp :=
          DateTimeToSQLTimeStamp(mtUserProp.FieldByName('RegDate').AsDateTime);
...
function TdmsProduct.Pkg_Info(const PkgNo: Integer;
  const SuppCode: String): String;
Begin
  ...
  Result := IntToStr(PkgNo) + '/' + SuppCode + ' It was created by ' +
    sp_PkgInfoCREATED_BY.AsString + ' at: ' + SQLTimeStampToStr('',
    sp_PkgInfoCREATED.AsSQLTimeStamp) + ' inventory: ' +
    sp_PkgInfoINVENTORY.AsString;
  sp_PkgInfo.Close;
End;
...
Begin
  ...
  sq_ProductLengths.SQL.Add
    ('FROM   dbo.ProductGroupLengths PGL INNER JOIN ProductLength PL ON PL.ProductLengthNo = PGL.ProductLengthNo');
...
Begin
  ...
  Begin
    ...
    Begin
      SQL := ' WHERE ((PGL.ProductGroupNo = ' +
        mtLO_RecordsPRODUCTGROUPNO.AsString + ')';
    End;
    mtLO_Records.Next;
    While not mtLO_Records.Eof do
    Begin
      SQL := SQL + ' OR (PGL.ProductGroupNo = ' +
        mtLO_RecordsPRODUCTGROUPNO.AsString + ')';
      mtLO_Records.Next;
...
begin
  ...
  if Self.InputOption = INPUT_PIECES then
    if InputOption <> 2 then
      For x := FirstLengthColumnInmatningViaLast to mtPackages.Fields.
        Count - 1 do
  ...
  Try
    ...
    Begin
      ...
      Try
        ...
        mtDupPkgNoSupplierCode.AsString :=
          Trim(DataSet.Fields[cSUPPLIERCODE].AsString);
        mtDupPkgNo.Post;
      Except
  ...
  Try
    ...
    Begin
      Try
        ...
        // April 20 2004
        sActualLengthMM := '0';
        mtPackages.FieldByName('PackageTypeNo').AsInteger :=
          GetPackageTypeNo(mtLO_RecordsPKGCODE.AsString, PCSPERPKG,
          sActualLengthMM);
        if (mtPackages.FieldByName('PackageTypeNo').AsInteger > 0) and
          (sActualLengthMM > '0') then
        ...
        mtPackages.FieldByName('ProductGroupNo').AsInteger :=
          mtLO_RecordsPRODUCTGROUPNO.AsInteger;
        mtPackages.FieldByName('ProductNo').AsInteger :=
          mtLO_RecordsProductNo.AsInteger;
        mtPackages.FieldByName('SupplierCode').AsString := SupplierCode;
        // removed TRIM sep 10
        mtPackages.FieldByName('Description').AsString :=
          mtLO_RecordsPRODUCTDESCRIPTION.AsString;
        mtPackages.FieldByName('ACT_THICK').AsFloat :=
          mtLO_RecordsACT_THICK.AsFloat;
        mtPackages.FieldByName('ACT_WIDTH').AsFloat :=
          mtLO_RecordsACT_WIDTH.AsFloat;
        mtPackages.FieldByName('NOM_THICK').AsFloat :=
          mtLO_RecordsNOM_THICK.AsFloat;
        mtPackages.FieldByName('NOM_WIDTH').AsFloat :=
          mtLO_RecordsNOM_WIDTH.AsFloat;
        mtPackages.FieldByName('INCH_THICK').AsFloat :=
          mtLO_RecordsINCH_THICK.AsFloat;
        mtPackages.FieldByName('INCH_WIDTH').AsFloat :=
          mtLO_RecordsINCH_WIDTH.AsFloat;
        mtPackages.FieldByName('SurfacingNo').AsInteger :=
          GetUTNo(mtLO_RecordsProductNo.AsInteger);
        mtPackages.FieldByName('OverrideRL').AsInteger :=
          mtLO_RecordsOverrideRL.AsInteger;
        mtPackages.FieldByName('Defsspno').AsInteger :=
          mtLO_RecordsSupplierShipPlanObjectNo.AsInteger;
        mtPackages.FieldByName('Varuslag').AsInteger :=
          mtLO_RecordsVaruSlag.AsInteger;

        if (Self.InputOption = INPUT_ARTICLES) and
          (mtLO_RecordsVaruSlag.AsInteger = 1) then
        Begin
          mtPackages.Fields.Fields[FirstLengthColumnInmatningViaLast].AsFloat :=
            mtLO_RecordsACT_WIDTH.AsFloat / 100;
        End;
...
begin
  if (((mtLO_RecordsACT_THICK.IsNull) or (mtLO_RecordsACT_THICK.AsFloat < 1) or
    (mtLO_RecordsACT_WIDTH.IsNull) or (mtLO_RecordsACT_WIDTH.AsFloat < 1)) AND
    (mtLO_RecordsVaruSlag.AsInteger <> 2)) then
    ...
    Begin
      ...
      Begin
        ...
        For x := FirstLengthColumnInmatningViaLast to mtPackages.Fields.
          Count - 1 do
        Begin
          mtPackages.Fields[cTOTALPCS].AsInteger := mtPackages.Fields[cTOTALPCS]
            .AsInteger + mtPackages.Fields[x].AsInteger;
        End;
        ...
        For x := FirstLengthColumnInmatningViaLast to mtPackages.Fields.
          Count - 1 do
        Begin
          mtPackages.Fields[cTOTALACTM3].AsFloat := mtPackages.Fields
            [cTOTALACTM3].AsFloat +
            (StrToFloat(mtPackages.Fields[x].DisplayLabel) / 1000
            // ActualLengthMMArray[x]/1000
            * mtPackages.Fields[x].AsInteger // Pieces
            * mtPackages.Fields[cACTTHICK].AsFloat / 1000 * mtPackages.Fields
            [cACTWIDTH].AsFloat / 1000);
        End;
        ...
        For x := FirstLengthColumnInmatningViaLast to mtPackages.Fields.
          Count - 1 do
          mtPackages.Fields[cTOTALLPM].AsFloat := mtPackages.Fields[cTOTALLPM]
            .AsFloat + (StrToFloat(mtPackages.Fields[x].DisplayLabel) / 1000
            // ActualLengthMMArray[x]/1000
            * mtPackages.Fields[x].AsInteger // Pieces
            );
        ...
        For x := FirstLengthColumnInmatningViaLast to mtPackages.Fields.
          Count - 1 do
          mtPackages.Fields[cTOTALM2].AsFloat := mtPackages.Fields[cTOTALM2]
            .AsFloat + (StrToFloat(mtPackages.Fields[x].DisplayLabel) / 1000
            // ActualLengthMMArray[x]/1000
            * mtPackages.Fields[x].AsInteger * // Pieces
      ...
      Begin
        // PCS
        if mtPackages.Fields[FirstLengthColumnInmatningViaLast].AsInteger < 1
        then
        ...
        For x := FirstLengthColumnInmatningViaLast to mtPackages.Fields.
          Count - 1 do
          mtPackages.Fields[cTOTALACTM3].AsFloat := mtPackages.Fields
            [cTOTALACTM3].AsFloat + (mtPackages.Fields[cTOTALLPM].AsFloat
        ...
        For x := FirstLengthColumnInmatningViaLast to mtPackages.Fields.
          Count - 1 do
          mtPackages.Fields[cTOTALM2].AsFloat := mtPackages.Fields[cTOTALM2]
            .AsFloat + (mtPackages.Fields[cTOTALLPM].AsFloat * // LÖPM
            mtPackages.Fields[cACTWIDTH].AsFloat / 1000);
      End
      else if Self.InputOption = INPUT_LOPM_AVERAGE_LENGTH then
      ...
      Begin
        ...
        For x := FirstLengthColumnInmatningViaLast to mtPackages.Fields.
          Count - 1 do
          mtPackages.Fields[cTOTALLPM].AsFloat := mtPackages.Fields
            [cTOTALLPM].AsFloat +
          // (NominalLengthMMArray[mtpackages.Fields[cRECID].AsInteger-1]
            (mtPackages.Fields.Fields[FirstLengthColumnInmatningViaLast].AsFloat
            * mtPackages.Fields[cTOTALPCS].AsInteger // Pieces
        ...
        For x := FirstLengthColumnInmatningViaLast to mtPackages.Fields.
          Count - 1 do
          mtPackages.Fields[cTOTALM2].AsFloat := mtPackages.Fields
            [cTOTALM2].AsFloat +
    ...
    Begin
      ...
      Begin
        ...
        mtPackages.Fields[cTOTALACTM3].AsFloat :=
          pi * Sqr(mtPackages.Fields[cACTTHICK].AsFloat / 100 / 2) *
          (mtPackages.Fields[cACTWIDTH].AsFloat / 100) * mtPackages.Fields
          [cTOTALPCS].AsInteger;
        ...
        // NOM M3
        mtPackages.Fields[cTOTALNOMM3].AsFloat := mtPackages.Fields
          [cTOTALACTM3].AsFloat;
        { mtpackages.Fields[cTOTALNOMM3].AsFloat:= 0 ;
          For x:= FirstLengthColumnInmatningViaLast to mtpackages.Fields.Count - 1 do
          mtpackages.Fields[cTOTALNOMM3].AsFloat:= mtpackages.Fields[cTOTALNOMM3].AsFloat +
        ...
        // LOPM
        mtPackages.Fields[cTOTALLPM].AsFloat := mtPackages.Fields[cTOTALPCS]
          .AsInteger * (mtPackages.Fields[cACTWIDTH].AsFloat / 100);
...
begin
  ...
  if (Self.InputOption = INPUT_ARTICLES) and (mtLO_RecordsVaruSlag.AsInteger = 1)
  then
    SummarizeArticles
  else
    SummarizePkg;
    ...
    Begin
      Try
        ...
        Begin
          ...
          if mtLO_Records.State in [dsEdit, dsInsert] then
            mtLO_Records.Post;
          ...
          End;
          if mtLO_Records.FindKey([mtPackages.FieldByName('Defsspno').AsInteger])
          then
          Begin
            mtLO_Records.Edit;
            mtLO_RecordsMATCH.AsInteger := 1;
...
function TdmsProduct.GetPackageTypeNo(Const PKGCODE: String;
  Var PCSPERPKG: Integer; Var sActualLengthMM: String): Integer;
Var
  PKGCODE_2: String;
Begin
...
Begin
  ...
  Begin
    ...
    if not mtLO_Records.Eof then
    Begin
      SQL := ' WHERE ((PL.ProductLengthNo = ' +
        mtLO_RecordsProductLengthNo.AsString + ')';
    End;
    mtLO_Records.Next;
    While not mtLO_Records.Eof do
    Begin
      SQL := SQL + ' OR (PL.ProductLengthNo = ' +
        mtLO_RecordsProductLengthNo.AsString + ')';
      mtLO_Records.Next;
...
procedure TdmsProduct.MakeLengthQuery_STD_Lengths(const STD_LengthGroupNo
  : Integer);
Begin
  sq_ProductLengths.SQL.Clear;
  sq_ProductLengths.SQL.Add
  ...
  sq_ProductLengths.SQL.Add('WHERE  PGL.GroupNo = ' +
    IntToStr(STD_LengthGroupNo));
  sq_ProductLengths.SQL.Add
    ('AND PL.ActualLengthMM > 0.5 AND PL.ActualLengthMM < 40000 ORDER BY PL.ActualLengthMM');
  // if thisuser.userid = 8 then sq_ProductLengths.SQL.SaveToFile('sq_ProductLengths.txt');
End;
...
Begin
  ...
  Begin
    ...
    if not mtLO_Records.Eof then
    Begin
      SQL := ' AND ((PrL.ProductGroupNo = ' +
        mtLO_RecordsPRODUCTGROUPNO.AsString + ')';
    End;
    mtLO_Records.Next;
    While not mtLO_Records.Eof do
    Begin
      SQL := SQL + ' OR (PrL.ProductGroupNo = ' +
        mtLO_RecordsPRODUCTGROUPNO.AsString + ')';
      mtLO_Records.Next;
...
Begin
  ...
  While not mtLO_Records.Eof do
  Begin
    if (mtLO_RecordsProductLengthNo.AsInteger = 453) or
      (mtLO_RecordsProductLengthNo.AsInteger = 533) or
      (mtLO_RecordsProductLengthNo.AsInteger = 1831) or
      (mtLO_RecordsProductLengthNo.AsInteger = 1960) or
      (mtLO_RecordsProductLengthNo.AsInteger = 3187) or
...
Begin
  ...
  Begin
    ...
    Begin
      SQL := 'WHERE  ((PGL.GroupNo = ' +
        IntToStr(mtLO_RecordsProductLengthGroupNo.AsInteger) + ')';
      Ett := True;
    End
    else
      SQL := SQL + ' OR (PGL.GroupNo = ' +
        IntToStr(mtLO_RecordsProductLengthGroupNo.AsInteger) + ')';
    mtLO_Records.Next;
  End;
  SQL := SQL + ')';
...
Begin
  if (DoesLOHaveFixedLength = True) and (DoesLOHaveValidLengthGroupNo = True)
  then
  Begin
    sq_ProductLengths.SQL.Clear;
  ...
  End
  else if (DoesLOHaveFixedLength = True) then
  Begin
    sq_ProductLengths.SQL.Clear;
    MakeLengthQuery_ForLOLengths_Only_ver2;
    sq_ProductLengths.SQL.Add('ORDER BY PL.ActualLengthMM ');
  End
  else if (DoesLOHaveValidLengthGroupNo = True) then
  Begin
    sq_ProductLengths.SQL.Clear;
    MakeLengthQuery_STD_Lengths_ver2;
    sq_ProductLengths.SQL.Add('ORDER BY PL.ActualLengthMM ');
...
function TdmsProduct.PackageNumberExists(const PkgNo: Integer;
  const SupplierCode: String3): Boolean;
begin
...
function TdmsProduct.SaveArticlePackages(const SaveToProdLogg, ProducerNo,
  LogicalInventoryPointNo, RegPointNo, RunNo: Integer;
  const WhenCreated: TDateTime): Boolean;
Var
  Save_Cursor: TCursor;
...
var
  ...
  Var
    ...
    begin
      // First get all package type (header) records that match
      sp_PackageTypesA.ParamByName('@ProductNo').AsInteger :=
        mtPackages.Fields[cPRODUCTNO].AsInteger;

      if mtPackages.Fields[cBARCODEID].AsInteger = 0 then
        sp_PackageTypesA.ParamByName('@BarCodeID').AsInteger := 0
      else
        sp_PackageTypesA.ParamByName('@BarCodeID').AsInteger :=
          mtPackages.Fields[cBARCODEID].AsInteger;
      if mtPackages.Fields[cGRADESTAMP].AsInteger = 0 then
        sp_PackageTypesA.ParamByName('@GradeStamp').AsInteger := 0
      else
        sp_PackageTypesA.ParamByName('@GradeStamp').AsInteger :=
          mtPackages.Fields[cGRADESTAMP].AsInteger;
      sp_PackageTypesA.ParamByName('@TotalPieces').AsInteger :=
        mtPackages.Fields[cTOTALPCS].AsInteger;
      sp_PackageTypesA.Open;
      try
        sp_PackageTypesA.First;
        while not sp_PackageTypesA.Eof do
        begin
          PkgTypeNumbers.Add
            (Pointer(sp_PackageTypesA.FieldByName('PackageTypeNo').AsInteger));
    ...
    Var
      ...
      ML: Double;
    Begin
      ML := mtPackages.FieldByName('ACT_WIDTH').AsFloat * 10;
      // (mtpackages.Fields[FirstLengthColumnInmatningViaLast].AsFloat*1000) ;
      Result := 0;
      New(ARecord);
      ARecord^.ProductLengthNo := dmsSystem.GetProductLengthNoByALMM
        (FloatToStr(ML));
    ...
    begin
      ...
      else if Self.InputOption = INPUT_ARTICLES then
      ...
      begin
        try
          ...
          sp_PackageTypeDetailA.ParamByName('@PackageTypeNo').AsInteger :=
            Integer(PkgTypeNumbers[0]);
          ...
          while LengthMatched and not sp_PackageTypeDetailA.Eof and
            (iDtl < MyList.Count) do
          begin
            ARecord := MyList.Items[iDtl];

            if (sp_PackageTypeDetailA.FieldValues['ProductLengthNo'] <>
              ARecord^.ProductLengthNo) OR
              (sp_PackageTypeDetailA.FieldValues['NoOfPieces'] <>
              ARecord^.NoOfPieces) then
              LengthMatched := False
            else
            begin
          ...
          if LengthMatched and sp_PackageTypeDetailA.Eof and
            (iDtl = MyList.Count) then
    ...
    begin
      ...
      Try
        ...
        sp_NewPackageTypeA.ParamByName('@PackageTypeNo').AsInteger :=
          PackagetypeNo;
        sp_NewPackageTypeA.ParamByName('@ProductNo').AsInteger :=
          mtPackages.Fields[cPRODUCTNO].AsInteger;

        if mtPackages.Fields[cBARCODEID].AsInteger = 0 then
          sp_NewPackageTypeA.ParamByName('@BarCodeID').AsInteger := 0
        else
          sp_NewPackageTypeA.ParamByName('@BarCodeID').AsInteger :=
            mtPackages.Fields[cBARCODEID].AsInteger;

        if mtPackages.Fields[cGRADESTAMP].AsInteger = 0 then
          sp_NewPackageTypeA.ParamByName('@GradeStamp').AsInteger := 0
        else
          sp_NewPackageTypeA.ParamByName('@GradeStamp').AsInteger :=
            mtPackages.Fields[cGRADESTAMP].AsInteger;
        sp_NewPackageTypeA.ParamByName('@TotalNoOfPieces').AsInteger :=
          mtPackages.Fields[cTOTALPCS].AsInteger;
      ...
      begin
        ARecord := MyList.Items[iDtl];
        Try
          sp_NewPackageDetailA.ParamByName('@PackageTypeNo').AsInteger :=
            PackagetypeNo;
          sp_NewPackageDetailA.ParamByName('@ProductLengthNo').AsInteger :=
            ARecord^.ProductLengthNo;
          sp_NewPackageDetailA.ParamByName('@NoOfPieces').AsInteger :=
            ARecord^.NoOfPieces;
          sp_NewPackageDetailA.ParamByName('@UserID').AsInteger :=
            ThisUser.UserID;
  ...
  begin
    ...
    Begin
      Try
        sp_NewPackageNoA.ParamByName('@PackageNo').AsInteger :=
          mtPackages.Fields[cPACKAGENO].AsInteger;
        sp_NewPackageNoA.ParamByName('@PackageTypeNo').AsInteger :=
          PackagetypeNo;
        sp_NewPackageNoA.ParamByName('@SupplierCode').AsString :=
          Trim(mtPackages.Fields[cSUPPLIERCODE].AsString);
        sp_NewPackageNoA.ParamByName('@LogicalInventoryPointNo').AsInteger :=
          LogicalInventoryPointNo;
        sp_NewPackageNoA.ParamByName('@SupplierNo').AsInteger := SupplierNo;
        sp_NewPackageNoA.ParamByName('@RegistrationPointNo').AsInteger :=
          RegPointNo;
        sp_NewPackageNoA.ParamByName('@UserID').AsInteger := ThisUser.UserID;
        sp_NewPackageNoA.ParamByName('@CreatedOfPkgStr').AsInteger := 0;
        // 0 = Manual entry
        sp_NewPackageNoA.ParamByName('@DateCreated').AsSQLTimeStamp :=
          DateTimeToSQLTimeStamp(WhenCreated);
    ...
    Begin
      Try
        sp_NewPkgNoNoProdLoggA.ParamByName('@PackageNo').AsInteger :=
          mtPackages.Fields[cPACKAGENO].AsInteger;
        sp_NewPkgNoNoProdLoggA.ParamByName('@PackageTypeNo').AsInteger :=
          PackagetypeNo;
        sp_NewPkgNoNoProdLoggA.ParamByName('@SupplierCode').AsString :=
          Trim(mtPackages.Fields[cSUPPLIERCODE].AsString);
        sp_NewPkgNoNoProdLoggA.ParamByName('@LogicalInventoryPointNo').AsInteger
          := LogicalInventoryPointNo;
        sp_NewPkgNoNoProdLoggA.ParamByName('@SupplierNo').AsInteger :=
          SupplierNo;
        sp_NewPkgNoNoProdLoggA.ParamByName('@RegistrationPointNo').AsInteger :=
          RegPointNo;
        sp_NewPkgNoNoProdLoggA.ParamByName('@UserID').AsInteger :=
          ThisUser.UserID;
        sp_NewPkgNoNoProdLoggA.ParamByName('@CreatedOfPkgStr').AsInteger := 0;
        // 0 = Manual entry
        sp_NewPkgNoNoProdLoggA.ParamByName('@DateCreated').AsSQLTimeStamp :=
          DateTimeToSQLTimeStamp(WhenCreated);
...
var
  ...
  Function GetLengthsEntered: String;
    ...
    Begin
      ...
      Begin
        ...
        mtImportedProductLengthsNoOfPcs.AsInteger :=
          sq_HTFPkgStatus.FieldByName('Styck').AsInteger;
        mtImportedProductLengthsProductLengthNo.AsInteger :=
          dmsSystem.GetProductLengthNoByLengthRule
          (mtImportedProductLengthsALMM.AsString,
  ...
  Var
    ...
    begin
      // First get all package type (header) records that match
      sp_PackageTypes.ParamByName('@ProductNo').AsInteger :=
        sq_HTFPkgStatus.FieldByName('ProductNo').AsInteger;

      if sq_HTFPkgStatus.FieldByName('BARCODEID').AsInteger = 0 then
        sp_PackageTypes.ParamByName('@BarCodeID').AsInteger := 0
      else
        sp_PackageTypes.ParamByName('@BarCodeID').AsInteger :=
          sq_HTFPkgStatus.FieldByName('BARCODEID').AsInteger;

      if sq_HTFPkgStatus.FieldByName('GRADESTAMP').AsInteger = 0 then
        sp_PackageTypes.ParamByName('@GradeStamp').AsInteger := 0
      else
        sp_PackageTypes.ParamByName('@GradeStamp').AsInteger :=
          sq_HTFPkgStatus.FieldByName('GRADESTAMP').AsInteger;
      sp_PackageTypes.ParamByName('@TotalPieces').AsInteger :=
        sq_HTFPkgStatus.FieldByName('TotalNoOfPieces').AsInteger;
  ...
  begin
    Try
      sp_vis_CngPkgTypeInPkg.ParamByName('@PackageNo').AsInteger :=
        sq_HTFPkgStatus.FieldByName('Paketnr').AsInteger;
      sp_vis_CngPkgTypeInPkg.ParamByName('@PackageTypeNo').AsInteger :=
        PackagetypeNo;
      sp_vis_CngPkgTypeInPkg.ParamByName('@SupplierCode').AsString :=
        Trim(sq_HTFPkgStatus.FieldByName('Prefix').AsString);
      sp_vis_CngPkgTypeInPkg.ParamByName('@UserID').AsInteger :=
        ThisUser.UserID;
      sp_vis_CngPkgTypeInPkg.ParamByName('@EventDate').AsSQLTimeStamp :=
  ...
  begin
    Try
      ...
      if (sq_HTFPkgStatus.FieldByName('Prefix').AsString = 'HTF') and
        ...
        sp_NewPackageNo.ParamByName('@LogicalInventoryPointNo')
          .AsInteger := 10668
      else
...
begin
  ...
  Try
    ...
    Begin
      if (not sq_HTFPkgStatus.FieldByName('ProductNo').IsNull) and
        (sq_HTFPkgStatus.FieldByName('ProductNo').AsInteger > 0) and
        (sq_HTFPkgStatusALMM.AsFloat > 0) and (not sq_HTFPkgStatusALMM.IsNull)
      then
      Begin
        ...
        try
          ...
          Try
            ...
            ShowMessage('Could not save Package no ' +
              Pkg_Info(sq_HTFPkgStatus.FieldByName('Paketnr').AsInteger,
              Trim(sq_HTFPkgStatus.FieldByName('Prefix').AsString)));
            Result := False;
            Raise;
          End;
      ...
      Begin
        Try
          ...
          try
            ...
            sq_InsTempPkgs.ParamByName('PackageNo').AsInteger :=
              cds_ImpProdMapIdentifier.AsInteger;
            sq_InsTempPkgs.ParamByName('SupplierCode').AsString :=
              cds_ImpProdMapClientCode.AsString;
