...
type
  ...
  private
    ...
    procedure SkapaEnKodFor_LO(const ShipPlanObjectNo, LanguageCode: Integer;
      const ELO: Boolean; const oThickness, oWidth, oLength,
      ArtikelKod: String);
    procedure CreateUnderKoderTillNollangd_for_ELO(const PkgCodePPZeroGroupNo,
      LanguageCode: Integer);
    procedure CreateUnderKoderTillNollangd_for_LO(const PkgCodePPZeroGroupNo
      : Integer);
    function SavePackageType: Boolean;
    Function GetPPNo_for_ADDLO(const ArtikelKod: String;
      const LanguageCode: Integer): Integer;
    Function GetPPNo(const ArtikelKod: String;
      const LanguageCode: Integer): Integer;
...
Var
  Save_Cursor: TCursor;
  // PIChanged,
  LayOutChanged, LOLengthChanged, ChangeCode: Boolean;
  x: Integer;
begin
  x := 0;
        ...
        Try
          ...
          Begin
            ...
            if cdsSawmillLoadOrdersProdInstruNo.IsNull then
              ...
              cds_PI_ELO.Filter := 'ProdInstruNo = ' +
                cdsSawmillLoadOrdersProdInstruNo.AsString;

            // PIChanged                   := PIChanged_for_specific_OrderRow ;

            cds_OLLayout.Filter := 'OrderLineNo = ' +
              cdsSawmillLoadOrdersOrderLineNo.AsString + ' AND OrderNo = ' +
              cdsSawmillLoadOrdersOrderNo.AsString;
            ...
            if ((cdsSawmillLoadOrders.UpdateStatus in [usModified, usInserted])
              or (LOLengthChanged) // or (LayOutChanged) or (PIChanged)
              or ((cdsSawmillLoadOrdersPkgCodePPNo.IsNull) or
              (cdsSawmillLoadOrdersPkgCodePPNo.AsInteger < 1))) then
            Begin
              ...
              Begin
                ...
                // ChangeCode := True ;
                if cdsSawmillLoadOrdersPcsPerPkg.OldValue <>
                  cdsSawmillLoadOrdersPcsPerPkg.NewValue then
                  ChangeCode := TRUE;
                if (cdsSawmillLoadOrdersPkgCodePPNo.IsNull) or
                  (cdsSawmillLoadOrdersPkgCodePPNo.AsInteger < 1) then
                  ChangeCode := TRUE;
                if cdsSawmillLoadOrdersShippingPlanStatus.AsInteger
                  in [STATUS_PRELIMINARY, STATUS_REJECTED, STATUS_COMPLETE,
                  STATUS_PRODUCTION_COMPLETE, STATUS_ANNULERAD,
                  STATUS_ANNULERAD_BEKRAFTAD] then
                  ChangeCode := False;
                if ChangeCode then
                Begin
                  cds_OLLayout.Active := False;
                  cds_OLLayout.ParamByName('OrderNo').AsInteger :=
                    cdsSawmillLoadOrdersOrderNo.AsInteger;
                  cds_OLLayout.ParamByName('OrderLineNo').AsInteger :=
                    cdsSawmillLoadOrdersOrderLineNo.AsInteger;
                  ...
                  mtProductTotalPcs.AsInteger :=
                    cdsSawmillLoadOrdersPcsPerPkg.AsInteger;
                  mtProduct.Post;

                  mtPkgLengths.Insert;
                  mtPkgLengthsProductLengthNo.AsInteger :=
                    cdsSawmillLoadOrdersProductLengthNo.AsInteger;
                  mtPkgLengths.Post;

                  SkapaEnKodFor_LO
                    (cdsSawmillLoadOrdersSupplierShipPlanObjectNo.AsInteger,
                    cdsSawmillLoadOrdersLanguageCode.AsInteger, False,
                    cdsSawmillLoadOrdersAT.AsString,
                    cdsSawmillLoadOrdersAB.AsString,
                    cdsSawmillLoadOrdersALMM.AsString,
                    cdsSawmillLoadOrdersRADREFERENS.AsString);

                  if cdsSawmillLoadOrdersPkgCodePPNo.AsInteger <>
                    mtPkgLengthsPkgCodePPNo.AsInteger then
                  Begin
                    // Räkna upp hur många huvudkoder som gjorts
                    InfoList.Add('Paketkod ändrad på LOnr/radID ' +
                      cdsSawmillLoadOrdersLONumber.AsString + '/' +
                      cdsSawmillLoadOrdersSupplierShipPlanObjectNo.AsString +
                      ' Variantnr= ' + mtPkgLengthsPkgCodePPNo.AsString);
                  End;

                  cdsSawmillLoadOrders.Edit;
                  cdsSawmillLoadOrdersSequenceNo.AsInteger :=
                    mtPkgLengthsPackageTypeNo.AsInteger;
                  cdsSawmillLoadOrdersPkgCodePPNo.AsInteger :=
                    mtPkgLengthsPkgCodePPNo.AsInteger;
                  cdsSawmillLoadOrdersPackageCode.AsString :=
                    mtPkgLengthsPkgCodePPNo.AsString;
                  cdsSawmillLoadOrders.Post;

                  if mtPkgLengthsPkgCodePPNo.AsInteger > 0 then
                    x := succ(x);

                  if (cdsSawmillLoadOrdersALMM.AsInteger <
                    1 { cds_LOALMM.AsFloat < 1 } ) and
                    (mtPkgLengthsPkgCodePPNo.AsInteger > 0) then
                    CreateUnderKoderTillNollangd_for_LO
                      (cdsSawmillLoadOrdersPkgCodePPNo.AsInteger);
      ...
      Try
        ...
        Try
          ...
          Begin
            ...
            cds_ELOLengths.ParamByName('SupplierShipPlanObjectNo').AsInteger :=
              cdsSawmillLoadOrdersSupplierShipPlanObjectNo.AsInteger;
            ...
            Begin
              ...
              Begin
                ...
                Begin
                  ...
                  cds_ELOLayout.ParamByName('SupplierShipPlanObjectNo')
                    .AsInteger :=
                    cdsSawmillLoadOrdersSupplierShipPlanObjectNo.AsInteger;
                  cds_ELOLayout.Active := TRUE;
                  ...
                  SkapaEnKodFor_LO
                    (cdsSawmillLoadOrdersSupplierShipPlanObjectNo.AsInteger,
                    LanguageCode, TRUE,
                    FloatToStr(cdsSawmillLoadOrdersAT.AsFloat),
                    FloatToStr(cdsSawmillLoadOrdersAB.AsFloat),
                    cdsSawmillLoadOrdersALMM.AsString,
                    cdsSawmillLoadOrdersRADREFERENS.AsString);
                  ...
                  Begin
                    // Räkna upp hur många huvudkoder som gjorts
                    InfoList.Add('Paketkod ändrad på ADD LOnr/radID ' +
                      cdsSawmillLoadOrdersLONumber.AsString + '/' +
                      cdsSawmillLoadOrdersSupplierShipPlanObjectNo.AsString +
                      ' Variantnr= ' + mtPkgLengthsPkgCodePPNo.AsString);
                  End;
                  ...
                  if mtPkgLengthsPkgCodePPNo.AsInteger > 0 then
                    x := succ(x);
                  if (cdsSawmillLoadOrdersALMM.AsFloat < 1) and
                    (mtPkgLengthsPkgCodePPNo.AsInteger > 0) then
                    CreateUnderKoderTillNollangd_for_ELO
                      (cdsSawmillLoadOrdersPkgCodePPNo.AsInteger, LanguageCode);
...
procedure TdmcOrder.SkapaEnKodFor_LO(const ShipPlanObjectNo,
  LanguageCode: Integer; const ELO: Boolean; const oThickness, oWidth, oLength,
  ArtikelKod: String);
Var
  PPNo: Integer;
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        cds_PPNooLength.AsString := oLength;
        // cds_OrderRowProdLength.AsString ;
    ...
    End // if SavePackageTypeResult then
      ...
      ShowMessage('Skapa kod misslyckades för LO radnr ' +
        inttostr(ShipPlanObjectNo));
  End; // With
End;

// Skapar underkoder till en nolllängd för LO
procedure TdmcOrder.CreateUnderKoderTillNollangd_for_ELO
  (const PkgCodePPZeroGroupNo, LanguageCode: Integer);
Begin
  // with dmPkgCode do
  Begin
    cds_ZeroGroup.Active := False;
    cds_ZeroGroup.ParamByName('PkgCodePPZeroGroupNo').AsInteger :=
      PkgCodePPZeroGroupNo;
    ...
    Begin
      ...
      mtPkgLengths.Insert;
      mtPkgLengthsProductLengthNo.AsInteger :=
        cds_ELOLengthsProductLengthNo.AsInteger;
      mtPkgLengths.Post;

      SkapaEnKodFor_LO(cdsSawmillLoadOrdersSupplierShipPlanObjectNo.AsInteger,
        LanguageCode, TRUE, FloatToStr(cdsSawmillLoadOrdersAT.AsFloat),
        FloatToStr(cdsSawmillLoadOrdersAB.AsFloat),
        cds_ELOLengthsActualLengthMM.AsString,
        cdsSawmillLoadOrdersReference.AsString);
      ...
      if not cds_ZeroGroup.Locate('PkgCodePPNo',
        mtPkgLengthsPkgCodePPNo.
        AsInteger { dmsSystem.cds_PPNoPkgCodePPNo.AsInteger } , []) then
      Begin
...
// Skapar underkoder till en nolllängd för LO
procedure TdmcOrder.CreateUnderKoderTillNollangd_for_LO
  (const PkgCodePPZeroGroupNo: Integer);
Begin
  // with dmPkgCode do
  Begin
    cds_ZeroGroup.Active := False;
    ...
    Begin
      ...
      SkapaEnKodFor_LO(cdsSawmillLoadOrdersSupplierShipPlanObjectNo.AsInteger,
        cdsSawmillLoadOrdersLanguageCode.AsInteger, False,
        cdsSawmillLoadOrdersAT.AsString, cdsSawmillLoadOrdersAB.AsString,
        cds_LOLengthsActualLengthMM.AsString,
        cdsSawmillLoadOrdersRADREFERENS.AsString);
...
Var
  Save_Cursor: TCursor;
  OLDPackageCodeNo, PackageCodeNo, OldIndexName: String;
...
var
  ...
  Var
    ...
    begin
      ...
      if mtProductGradeStampNo.AsInteger < 0 then
        ...
        sp_PackageTypes.ParamByName('@GradeStamp').AsInteger :=
          mtProductGradeStampNo.AsInteger;
      ...
      try
        ...
        while not sp_PackageTypes.Eof do
        begin
          PkgTypeNumbers.Add
            (Pointer(sp_PackageTypes.FieldByName('PackageTypeNo').AsInteger));
    ...
    Begin
      Result := 0;
      New(ARecord);
      ARecord^.ProductLengthNo := mtPkgLengthsProductLengthNo.AsInteger;
      // StrToInt(mtPackages.Fields[x].FieldName) ;
      ARecord^.NoOfPieces := mtProductTotalPcs.AsInteger;
      // mtPkgLengthsPieces.AsInteger ;// mtPackages.Fields[x].AsInteger ;
      MyList.Add(ARecord);
    ...
    begin
      ...
      begin
        try
          ...
          sp_PackageTypeDetail.ParamByName('@PackageTypeNo').AsInteger :=
          ...
          while LengthMatched and not sp_PackageTypeDetail.Eof and
          ...
          if LengthMatched and sp_PackageTypeDetail.Eof and (iDtl = MyList.Count)
          then
    ...
    begin
      ...
      Try
        ...
        sp_NewPackageType_II.ParamByName('@PackageTypeNo').AsInteger :=
          PackageTypeNo;
        sp_NewPackageType_II.ParamByName('@ProductNo').AsInteger :=
          mtProductProductNo.AsInteger;
        if mtProductBarCodeNo.AsInteger < 0 then
          sp_NewPackageType_II.ParamByName('@BarCodeID').AsInteger := 0
        else
          sp_NewPackageType_II.ParamByName('@BarCodeID').AsInteger :=
            mtProductBarCodeNo.AsInteger;

        if mtProductGradeStampNo.AsInteger < 0 then
          sp_NewPackageType_II.ParamByName('@GradeStamp').AsInteger := 0
        else
          sp_NewPackageType_II.ParamByName('@GradeStamp').AsInteger :=
            mtProductGradeStampNo.AsInteger;
        sp_NewPackageType_II.ParamByName('@TotalNoOfPieces').AsInteger :=
          mtProductTotalPcs.AsInteger;

        sp_NewPackageType_II.ParamByName('@UserID').AsInteger :=
          ThisUser.UserID;
        sp_NewPackageType_II.ParamByName('@ProdInstruNo').AsInteger := -1;
        // dmsSystem.cds_ProdInstructProdInstruNo.AsInteger ;
...
Function TdmcOrder.GetPPNo_for_ADDLO(const ArtikelKod: String;
  const LanguageCode: Integer): Integer;
...
Begin
  ...
  Begin
    Result := -1;
    sq_GetPPNo.ParamByName('PackageTypeNo').AsInteger :=
      mtPkgLengthsPackageTypeNo.AsInteger;
    ...
    Try
      ...
      Begin
        ...
        cds_PP_Layout.ParamByName('PkgCodePPNo').AsInteger :=
          sq_GetPPNoPkgCodePPNo.AsInteger;
        cds_PP_Layout.Active := TRUE;

        if (cds_ELOLayout.RecordCount = 0) and (cds_PP_Layout.RecordCount = 0)
        then
          Result := sq_GetPPNoPkgCodePPNo.AsInteger
        else if cds_ELOLayout.RecordCount = cds_PP_Layout.RecordCount then
        Begin
          NoGood := False;
          cds_ELOLayout.First;
          While not cds_ELOLayout.Eof do
          Begin
            if not cds_PP_Layout.Locate('LayoutNo',
              cds_ELOLayoutLayoutNo.AsInteger, []) then
            Begin
              Result := -1;
              NoGood := TRUE;
              Break;
            End;
            if cds_ELOLayoutAntalKopior.AsInteger = cds_PP_LayoutAntalKopior.AsInteger
            then
              Result := sq_GetPPNoPkgCodePPNo.AsInteger
            else
            Begin
...
Function TdmcOrder.GetPPNo(const ArtikelKod: String;
  const LanguageCode: Integer): Integer;
// Var PkgCodePPNo : Integer ;
Var
  NoGood: Boolean;
  ...
  Begin
    ...
    Try
      ...
      Begin
        ...
        if (cds_OLLayout.RecordCount = 0) and (cds_PP_Layout.RecordCount = 0)
        then
          Result := sq_GetPPNoPkgCodePPNo.AsInteger
        else if cds_OLLayout.RecordCount = cds_PP_Layout.RecordCount then
        Begin
          NoGood := False;
          cds_OLLayout.First;
          ...
          Begin
            if not cds_PP_Layout.Locate('LayoutNo',
              cds_OLLayoutLayoutNo.AsInteger, []) then
            Begin
              Result := -1;
              NoGood := TRUE;
              Break;
            End;
            if cds_OLLayoutAntalKopior.AsInteger = cds_PP_LayoutAntalKopior.AsInteger
            then
              Result := sq_GetPPNoPkgCodePPNo.AsInteger
            else
            Begin
...
procedure TdmcOrder.RemoveCodeFromZeroGroup(const PkgCodePPZeroGroupNo,
  PkgCodePPNo: Integer);
Begin
  Try
    sq_RemFromZeroGroup.ParamByName('PkgCodePPZeroGroupNo').AsInteger :=
      PkgCodePPZeroGroupNo;
...
Function TdmcOrder.GetTheLO_SupplierNo(const SupplierNo, LONo: Integer)
  : Integer;
// OM BTH eller BTB och båda finns i LO then fråga vem som är leverantör
var
  fSelectBTHorBTB: TfSelectBTHorBTB;
Begin
  Try
    sq_CheckSupplierNo.ParamByName('LONo').AsInteger := LONo;
    sq_CheckSupplierNo.Open;
    if ((not sq_CheckSupplierNo.Eof) and
      (sq_CheckSupplierNoNoOfSuppliers.AsInteger > 1) and
      ((SupplierNo = 2878) or (SupplierNo = 172))) then
    Begin
