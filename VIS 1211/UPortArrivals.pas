...
//    Function IsLoadValid(LoadNo, ShippingPlanNo, ObjectType : Integer;Sender: TObject) : Boolean ;
    procedure PrintSamlingsspecifikationPerRef(Sender: TObject;const SamLastNr : Integer);
...
//Här hämta vi upp verkets LoadNo om det finns, annars visas VW LoadNo
 ...
 cdsPortArrivingLoads.SQL.Add('INNER JOIN dbo.Loaddetail LD2		ON 	LD2.LoadNo = LSP2.LoadNo') ;
 cdsPortArrivingLoads.SQL.Add('						and LD2.ShippingPlanNo = LSP2.ShippingPlanNo') ;

 cdsPortArrivingLoads.SQL.Add('INNER JOIN dbo.CustomerShippingPlanDetails CSD2 	ON CSD2.CustShipPlanDetailObjectNo = LD2.DefaultCustShipObjectNo') ;
 cdsPortArrivingLoads.SQL.Add('INNER JOIN dbo.OrderLine	OL2 ON OL2.OrderNo = CSD2.OrderNo AND OL2.OrderLineNo = CSD2.OrderLineNo') ;
...
// cdsPortArrivingLoads.SQL.Add('AND     L.CustomerNo 		= SP.CustomerNo');
 ...
 cdsPortArrivingLoads.SQL.Add('WHERE Bk.ShippingCompanyNo          = ' + cds_PropsShipperNo.AsString);
...
Var FormCRViewReport : TFormCRViewReport ;
     A                 : array of variant ;
begin
 if dmArrivingLoads.cdsPortArrivingLoadsVerk_LoadNo.AsInteger < 1 then exit ;
...
procedure TfrmPortArrivals.PrintSamlingsspecifikationPerRef(Sender: TObject;const SamLastNr : Integer);
Var FormCRViewReport : TFormCRViewReport ;
     A                 : array of variant ;
begin
 if dmArrivingLoads.cdsPortArrivingLoadsVerk_LoadNo.AsInteger < 1 then exit ;
...
//      ShowMessage(E.Message);
  ...
  Try
   ...
   Begin
    ...
    ColIdx      := grdPortLoadsDBTableView1.DataController.GetItemByFieldName('Verk_LoadNo').Index;
    Verk_LoadNo := grdPortLoadsDBTableView1.DataController.Values[RecIdx, ColIdx];

    ColIdx      := grdPortLoadsDBTableView1.DataController.GetItemByFieldName('MÄRKNING').Index;
    Reference   := grdPortLoadsDBTableView1.DataController.Values[RecIdx, ColIdx];
...
//Utskriften styrs av VW lastnr, i SP skall verkets lastnr visas om sådant finns
  dmsSystem.sq_PkgType_InvoiceByCSD.ParamByName('LoadNo').AsInteger := dmArrivingLoads.cdsPortArrivingLoadsVerk_LoadNo.AsInteger ;
...
//      ShowMessage(E.Message);
 ...
 End ;
  Try
  dmsSystem.sq_DelPkgType.ParamByName('LoadNo').AsInteger:= dmArrivingLoads.cdsPortArrivingLoadsVerk_LoadNo.AsInteger ;
...
begin
 ...
 Begin
  Try
  dmsSystem.sq_PkgType_InvoiceByLO.ParamByName('LoadNo').AsInteger:= dmArrivingLoads.cdsPortArrivingLoadsVerk_LoadNo.AsInteger ;
  dmsSystem.sq_PkgType_InvoiceByLO.ExecSQL ;
     except
      On E: Exception do
      Begin
...
begin
 acSamlingsFSPerRef.Enabled:= grdPortLoadsDBTableView1.Controller.SelectedRecordCount > 0 ;
end;

procedure TfrmPortArrivals.acSamlingsFS_USAUpdate(Sender: TObject);
begin
 acSamlingsFS_USA.Enabled:= grdPortLoadsDBTableView1.Controller.SelectedRecordCount > 0 ;
end;

procedure TfrmPortArrivals.acPrintDirectFSUpdate(Sender: TObject);
begin
 acPrintDirectFS.Enabled:= grdPortLoadsDBTableView1.Controller.SelectedRecordCount > 0 ;
end;

procedure TfrmPortArrivals.acPrintTallyUSNoteUpdate(Sender: TObject);
begin
 acPrintTallyUSNote.Enabled:= grdPortLoadsDBTableView1.Controller.SelectedRecordCount > 0 ;
end;

procedure TfrmPortArrivals.acPrintDirectFS_USAUpdate(Sender: TObject);
begin
 acPrintDirectFS_USA.Enabled:= grdPortLoadsDBTableView1.Controller.SelectedRecordCount > 0 ;
end;
