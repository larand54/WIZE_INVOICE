...
procedure set_Print(report_Name: string; rpt_FilePath: string; report_Option:
  string; print_Type: string; store_Path: string; pdf_Name: string;
  safeArray: PVarArray; server_Name: string; database_Name: string;
  userID: string; password: string; iSecurity: Boolean);
   ...
   //   Try
      if HYModuleManager2.ModuleCount = 0 then
         begin
             ...
             HYModuleManager2.CreateVisualPlugin('CrystalReport_Plugin.CR_Plugin', fVisualizer1);
         end;
end;
...
procedure set_Print;
  ...
  begin
              ...
              with (fVisualizer1 as PluginInterface) do begin
                set_Print(report_Name, rpt_FilePath, report_Option, print_Type, store_Path, pdf_Name, safeArray, server_Name, database_Name, userID, password, iSecurity);
              end;
