...
procedure TfUserPreference.beFragaAvropDirPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  With dmsSystem do
  Begin
    if cxShellBrowserDialog1.Execute then
      cds_PropsUserDir.AsString := IncludeTrailingPathDelimiter
        (cxShellBrowserDialog1.Path);
  End;
end;

procedure TfUserPreference.cxDBButtonEdit1PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  With dmsSystem do
  Begin
    if cxShellBrowserDialog1.Execute then
      cds_PropsExcelDir.AsString := IncludeTrailingPathDelimiter
        (cxShellBrowserDialog1.Path);
  End;
end;
...
begin
  ...
  Begin
    if cxShellBrowserDialog1.Execute then
      cds_PropsLODir.AsString := IncludeTrailingPathDelimiter
        (cxShellBrowserDialog1.Path);
  End;
end;
...
begin
  ...
  Begin
    if cxShellBrowserDialog1.Execute then
      cds_PropsWOODXDir.AsString := IncludeTrailingPathDelimiter
        (cxShellBrowserDialog1.Path);
  End;
end;
