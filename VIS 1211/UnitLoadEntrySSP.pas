...
Var x, LocalSupplierNo  : Integer ;
    frmPackageEntry     : TfrmPackageEntry ;
begin
 LocalSupplierNo := dmsContact.GetLocalSupplierNo(dmLoadEntrySSP.cds_LSPLoadingLocationNo.AsInteger) ;
 if LocalSupplierNo > 0 then
 Begin
   if dmLoadEntrySSP.cds_LoadHeadLoadNo.AsInteger > 0 then
   Begin
   if (Length(lcPIP.Text) > 0) and (Length(lcLIP.Text) > 0) and (dmLoadEntrySSP.cds_LoadHeadLIPNo.AsInteger > 0) and (dmLoadEntrySSP.cds_LoadHeadLIPNo.IsNull = False) then
   Begin
  ...
  {    if dmsContact.IsClientLego(ThisUser.CompanyNo) = 6 then
      ...
      dmsProduct.SupplierCode:= dmsContact.GetClientCode(LocalSupplierNo) ;
...
//Långsamt här
    ...
    Begin
     ShowMessage('Paketnr ' + IntToStr(NewPkgNo) + ' finns inte i lager ' + Trim(lcPIP.Text)) ;
     Error      := True ;
    End
    else
    Begin
