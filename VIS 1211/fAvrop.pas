...
Function GetInvoiceDocTextByOrderNoLocal_III(const OrderNos, LONos : String) : Variant ;
Var S, L : string ;
    rtfString : ansistring ;
    stream    : TMemoryStream ;
    x : integer ;
...
Begin
 ...
 Begin
  ...
  sq_GetInvTexts.SQL.Add('Inner Join dbo.DocTypeOrder DTO on DTO.OrderNo = DT.OrderNo') ;
  ...
  sq_GetInvTexts.SQL.Add('Inner Join dbo.DocTypeOrder_Avrop DTO on DTO.LONo = DT.LONo') ;
  sq_GetInvTexts.SQL.Add('AND DTO.DocTextNo = DT.DocTextNo') ;
 ...
 begin
  ...
  Try
   ...
   if cdsInvoiceHead.State in [dsBrowse] then
    ...
    GetInvoiceDocTextByOrderNoLocal_III (Ordernos.CommaText, LONos.CommaText) ;
...
{   if Self.cds_PropsOrderTypeNo.AsInteger = 0 then
   ...
   cdsInvoiceHeadInvoiceType.AsInteger                           := GetInvoiceTypeFromBinding(sq_GetInvoiceHeadDataSalesRegionNo.AsInteger, sq_GetInvoiceHeadDataOrderType.AsInteger) ;
   ...
   cdsInvoiceHeadSupplierNo.AsInteger                            := sq_GetInvoiceHeadDataSalesRegionNo.AsInteger ;// daMoLM1.cdsAvropSALESREGIONNO.AsInteger ;//VIDA_WOOD_CLIENTNO ; //Vida Wood mars 16 2006
...
//    ShowMessage('Hello cdsInvoiceHeadInvoiceText');
   ...
   if (cdsInvoiceHeadFreeDelMonth.AsInteger = 1) or (cdsInvoiceHeadBLDueDate.AsInteger = 1) then
   Begin
...
//    sq_GetPkgType_Invoice.ParamByName('InternalInvoiceNo').AsInteger := InternalInvoiceNo ;
    ...
    CASE cds_CSH_ExtraCostInvoiceAdditionUnitNo.AsInteger of
     2 :  TempVal := sq_GetPkgType_InvoicePcs.AsFloat ;
     3 :  TempVal := sq_GetPkgType_InvoiceAM3.AsFloat ;
     4 :  TempVal := sq_GetPkgType_InvoiceNM3.AsFloat ;
     5 :  TempVal := sq_GetPkgType_InvoiceAdNl.AsFloat ;
     6 :  TempVal := sq_GetPkgType_InvoiceNdAl.AsFloat ;
     7 :  TempVal := sq_GetPkgType_InvoiceAM2AW.AsFloat ;
     8 :  TempVal := sq_GetPkgType_InvoiceAM2AW.AsFloat ;
     9 :  TempVal := sq_GetPkgType_InvoiceAM1.AsFloat ;
    10 :  TempVal := 0 ;
    11 :  TempVal := 1 ;
    12 :  TempVal := sq_GetPkgType_InvoiceNM1.AsFloat ;
    END;
...
Begin
 ...
 begin
  Try
   ...
   cds_CSH_ExtraCost.ParamByName('LONo').AsInteger  := mtCompSelRowsLONo.AsInteger ;
   ...
   Begin
    cdsInvoiceDetail.Insert ;
    cdsInvoiceDetailShippingPlanNo.AsInteger            := mtCompSelRowsLONo.AsInteger ;
    cdsInvoiceDetailTypeOfRow.AsInteger                 := 2 ; //Additional cost
    cdsInvoiceDetailPriceUnit.AsString                  := cds_CSH_ExtraCostPriceUnit.AsString ;
    cdsInvoiceDetailProductDescription.AsString         := cds_CSH_ExtraCostInvoiceAdditionDescription.AsString ;


    SekPrice                                            := cds_CSH_ExtraCostInvoiceAdditionAmount.AsFloat * dmsSystem.GetExRate(cds_CSH_ExtraCostInvoiceAdditionCurrencyNo.AsInteger) ;

    cdsInvoiceDetailPrice.AsFloat                       := SekPrice / dmsSystem.GetExRate(cdsInvoiceHeadCurrencyNo.AsInteger) ;

    cdsInvoiceDetailVolumeUnit.AsString                 := cds_CSH_ExtraCostPriceUnit.AsString ;//Get unit name ;
    cdsInvoiceDetailArticleNo.AsInteger                 := cds_CSH_ExtraCostArticleNo.AsInteger ;
    cdsInvoiceDetailInclInPrice.AsInteger               := 0 ;
    cdsInvoiceDetailInclInInvoiceTotal.AsInteger        := 1 ;
    TempVal  := 0 ;
//    if (cdsInvoiceLOInvoiceAdditionAmount.AsFloat > 0) or (cdsInvoiceLOInvoiceAdditionAmount.AsFloat < 0) then
...
Begin
 ...
 Begin
  ...
  Begin
   ...
   sp_LdConnCtrl.ParamByName('@LONo').AsInteger :=  mtCompSelRowsLONo.AsInteger ;
...
begin
 ...
 Begin
  ...
  if fSelectLIPNo.ShowModal = mrOK then
   Begin
    if (not fSelectLIPNo.mtPropsLIPNo.IsNull) and (fSelectLIPNo.mtPropsLIPNo.AsInteger > 0) then
    Begin
     dmsSystem.MovePkgToInvFromLoad (daMoLM1.cdsLoadsLoadNo.AsInteger, fSelectLIPNo.mtPropsLIPNo.AsInteger) ;
    End
     else
      ShowMessage('Kan inte flytta paket pga att ingen lagergrupp är vald.') ;
   End;
  Finally
...
begin
 ...
 Try
  SetLength(A, 2);
  A[0]  := grdLODBTableView1.DataController.DataSource.DataSet.FieldByName('ShippingPlanNo').AsInteger ;
  A[1]  := grdLODBTableView1.DataController.DataSource.DataSet.FieldByName('SUPPLIERNO').AsInteger ;
 FormCRViewReport.CreateCo('LASTORDER_NOTE_ver3.RPT', A) ;
