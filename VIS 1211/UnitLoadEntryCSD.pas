...
uses
  ...
  uADCompClient, DateUtils, dxSkinsCore, dxSkinBlack, dxSkinBlue,
...
//check that no user has reserved the package
        ...
        PkgSupplierCode,
        'TfLoadEntryCSD', //Self.Name,
        Res_UserName          ) = ThisUser.UserName+'/'+'TfLoadEntryCSD' //{ NO_USER_HAS_THIS_PACKAGE_RESERVED }
        then begin
          Result := eaACCEPT ;
          end
        else
...
begin
    ...
    End
        ...
        if dmsSystem.Pkg_Reserved(
          PkgNo,
          PkgSupplierCode, 'TfLoadEntryCSD', Res_UserName
          ) = ThisUser.UserName+'/'+'TfLoadEntryCSD' { NO_USER_HAS_THIS_PACKAGE_RESERVED }then begin
          Result := eaACCEPT ;
          end
        else
...
//   if cds_LoadPackages.Locate('LoadNo;LoadDetailNo', VarArrayOf([cds_LoadHeadLoadNo.AsInteger, mtLoadPackagesLoadDetailNo.AsInteger]),[]) then
  if cds_LoadPackages.FindKey([cds_LoadHeadLoadNo.AsInteger, mtLoadPackagesLoadDetailNo.AsInteger]) then
   Begin
...
//   dmLoadEntryCSD.LogicalTransferInventoryNo:=    dmLoadEntryCSD.cds_LoadHeadPIPNo.AsInteger ;
   ...
   if cds_LoadHead.State in [dsEdit, dsInsert] then
   cds_LoadHead.Post ;
   if (cds_LoadPackages.State in [dsEdit, dsInsert])
   and ((not cds_LoadPackagesPackageNo.IsNull) and (cds_LoadPackagesPackageNo.AsInteger > 0)) then
   cds_LoadPackages.Post
   else
   cds_LoadPackages.Cancel ;
   cds_LoadPackages.DisableControls ;
  ...
  if (cds_LoadHeadSenderLoadStatus.AsInteger = 2) and (not ControlInvDate) and (DoingCreditingPkgs = False) then
  Begin
...
begin
 ...
 Case Pkg_Function of
   ...
   CREDIT_PKG_FROM_LOAD : AStyle  := cxStyleFuchsia ;
 End ;
end;
...
//  dmsProduct.InventoryNo      := dmLoadEntryCSD.cds_LSPShipToInvPointNo.AsInteger ;
  ...
  dmsContact.GetClientCode(dmLoadEntryCSD.cds_LSPSalesRegionNo.AsInteger) ;
...
Var Save_Cursor   : TCursor;
    ...
    NewLO         : Integer ;
Begin
 if MessageDlg('Vill du kreditera markerade paket?',
 mtConfirmation, [mbYes, mbNo], 0) = mrYes then
 With dmLoadEntryCSD do
 Begin
  ...
  Try
    ...
    Try
      ...
      if MessageDlg('Vill du skapa preliminär kreditfaktura?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      CreateCreditInvoice ;

      if MessageDlg('Vill du skapa intern LO?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      Begin
