...
type
  ...
  private
    ...
    procedure SetCheckComboBoxes_Where_PktNrLevKod_Required (const VerkNo : Integer) ;
...
begin
 ...
 A[1]:= grdLODBTableView1.DataController.DataSet.FieldByName('Supplier').AsInteger ;
...
Function TfrmLoadOrder.GetSQLofComboFilter(const dType : Byte;const Kolumn : String;combo : TcxCheckComboBox) : String ;
Var
...
Begin
 ...
 Begin
  ...
  cdsSawmillLoadOrders.SQL.Add('AND SP.StartETDYearWeek >= ' + teFromWeek.Text) ;
  if Length(teToWeek.Text)  > 0 then
  cdsSawmillLoadOrders.SQL.Add('AND SP.EndETDYearWeek <= ' + teToWeek.Text) ;


  cdsSawmillLoadOrders.SQL.Add(GetSQLofComboFilter(0, 'pli.GradeCode', ccbKV2)) ;
  cdsSawmillLoadOrders.SQL.Add(GetSQLofComboFilter(0, 'pli.SurfacingCode', ccbSU2)) ;
  cdsSawmillLoadOrders.SQL.Add(GetSQLofComboFilter(0, 'pli.VaruGruppNo', ccVarugrupp)) ;
//  cdsSawmillLoadOrders.SQL.Add(GetSQLofComboFilter(0, 'imp.ImpCode', ccbIMP)) ;
  cdsSawmillLoadOrders.SQL.Add(GetSQLofComboFilter(0, 'od.IntraStatCountryNo', ccLand)) ;
  cdsSawmillLoadOrders.SQL.Add(GetSQLofComboFilter(0, 'CH.CustomerNo', ccKund)) ;
...
//       sp_SawmillLoadOrders.SQL.Add('  AND SP.ShippingPlanStatus <> 4 ' ) ;
  ...
  1 : Begin //all except complete, ALLA UTOM KLAR OCH ANNULERING BEKRÄFTAD
       cdsSawmillLoadOrders.SQL.Add('  AND SP.ShippingPlanStatus <> 0 AND SP.ShippingPlanStatus <> 4 AND SP.ShippingPlanStatus <> 8' ) ;
      End ;

  2 : Begin //complete KLAR OCH ANNULERING BEKRÄFTAD
       cdsSawmillLoadOrders.SQL.Add('  AND (SP.ShippingPlanStatus = 0 OR SP.ShippingPlanStatus = 4 OR SP.ShippingPlanStatus = 8)' ) ;
      End ;

  3 : Begin //NEW AND ANNULERAD
       cdsSawmillLoadOrders.SQL.Add('  AND (SP.ShippingPlanStatus = 0 OR SP.ShippingPlanStatus = 1 OR SP.ShippingPlanStatus = 10 OR SP.ShippingPlanStatus = 7) ' ) ;
      End ;
  4 : Begin //reject
 ...
 11 : Begin //Prel, Ny och Accept
       cdsSawmillLoadOrders.SQL.Add('  AND (SP.ShippingPlanStatus = 0 OR SP.ShippingPlanStatus = 1 OR SP.ShippingPlanStatus = 10 OR SP.ShippingPlanStatus = 3) ' ) ;
      End ;
...
//  cdsSawmillLoadOrders.SQL.Add('END END AS Pris,') ;
  ...
  cdsSawmillLoadOrders.SQL.Add('sp.ProductNo, sp.ProductLengthNo, Od.LanguageCode AS LanguageCode, PL.ActualLengthMM AS ALMM, SP.SequenceNo, SP.OrderLineNo, SP.OrderNo, SP.ModifiedUser, LIP.LogicalInventoryName AS Lagergrupp, SP.LengthSpec AS Längd,') ;
  cdsSawmillLoadOrders.SQL.Add('StatCtry.CountryName AS Land, SP.Package_Size, ps.PackageSizeName,') ;

  cdsSawmillLoadOrders.SQL.Add('SP.PkgArticleNo, SP.LOGroupNo, LOB.LOBuffertName,') ;
  ...
  if dmcOrder.cds_PropsOrderTypeNo.AsInteger = 0 then
  cdsSawmillLoadOrders.SQL.Add('INNER JOIN dbo.Client_LoadingLocation       CLL   ON   CLL.PhyInvPointNameNo = SP.LoadingLocationNo')
  else
  cdsSawmillLoadOrders.SQL.Add('INNER JOIN dbo.Client_LoadingLocation       CLL   ON  CLL.PhyInvPointNameNo = SP.ShipToInvPointNo') ;


  cdsSawmillLoadOrders.SQL.Add('LEFT Join dbo.PackageSize ps on ps.PackageSizeNo = SP.Package_Size') ;
  ...
  cdsSawmillLoadOrders.SQL.Add('Left Join dbo.Barcode BC ON BC.BarCodeNo = PIS.BarCodeID') ;
  cdsSawmillLoadOrders.SQL.Add('ON PIS.ProdInstruNo = SP.ProdInstructNo') ;

  cdsSawmillLoadOrders.SQL.Add('INNER JOIN dbo.Country StatCtry	ON StatCtry.CountryNo  = od.IntraStatCountryNo') ;
...
//Prislistan används inte på inköp!
  ...
  cdsSawmillLoadOrders.SQL.Add('SP.PcsPerPkg, SP.PackageWidth, SP.PackageHeight, SP.PkgCodePPNo, PIS.ProdInstruNo,') ;
  cdsSawmillLoadOrders.SQL.Add('sp.ProductNo, sp.ProductLengthNo, 1 AS LanguageCode, PL.ActualLengthMM AS ALMM, SP.SequenceNo, SP.OrderLineNo, SP.OrderNo, SP.ModifiedUser, LIP.LogicalInventoryName AS Lagergrupp, SP.LengthSpec AS Längd,') ;
  cdsSawmillLoadOrders.SQL.Add('StatCtry.CountryName AS Land, SP.Package_Size, ps.PackageSizeName,') ;
  ...
  cdsSawmillLoadOrders.SQL.Add('LEFT OUTER JOIN dbo.Orders Od ON Od.OrderNo = SP.OrderNo') ;
...
begin
 ...
 or (Length(Trim(teReferens.Text)) > 0) then
  Begin
...
cdsSawmillLoadOrders.SQL.Add('Left Outer Join dbo.Booking Bk On BK.ShippingPlanNo = SP.ShippingPlanNo') ;
  ...
  Begin
   cdsSawmillLoadOrders.SQL.Add('WHERE SP.Reference = ' + QuotedStr(teReferens.Text)) ;
  End
    else
...
{  if (dmsContact.IsClientLego(ThisUser.CompanyNo) = cLego)
  ...
  Begin
   cdsSawmillLoadOrders.SQL.Add('WHERE SP.Reference = ' + QuotedStr(teReferens.Text)) ;
   cdsSawmillLoadOrders.SQL.Add('AND  CLL.ClientNo = ' + dmcOrder.cds_PropsVerkNo.AsString) ;
  End
    else
     cdsSawmillLoadOrders.SQL.Add('WHERE  SP.ShippingPlanNo = ' + teSearchLONo.Text) ;
...
begin
 acShowPriceList.Enabled:= (dmcOrder.cdsSawmillLoadOrders.Active)
 and (Length(dmcOrder.cdsSawmillLoadOrdersPriceListName.AsString) > 0 ) ;
end;
...
procedure TfrmLoadOrder.AddMarkedLOToFile (Sender: TObject;const Mapp : String;const NewFile : Boolean) ;
Var S : TStringList ;
Begin
...
procedure TfrmLoadOrder.teSearchLoadNoKeyDown(Sender: TObject;
...
procedure TfrmLoadOrder.CreateLoadAndEntryTo_Load_Imp(const L_SupplierNo, LONo, TempLoadNo : Integer;const Fil, Mapp : String) ;
Var S   : TStringList ;
    SPN,
    EgnaPaket,
    LoadID, FS,
...
// Mapp := dmsSystem.Get_A_Value_From_UserDir('LastOrder.txt', 'ExportDir') ;
 ...
 Try
  ...
  Begin
   ...
   Begin
    Prefix:= dmsContact.GetSuppliercodeByPktLevKod (Copy(S.Strings[x], 12,2)) ;
...
//      Prefix := dmcOrder.CheckIfChangeToPrefix_BH1(PkgNo, L_SupplierNo, L_SupplierNo, LONo, Prefix) ;
     ...
     Begin
      ...
      Prefix  := dmsSystem.PkgNoToSuppCodeImportedPkg(PkgNo, L_SupplierNo, L_SupplierNo, LONo, SupplierNo, ProductNo) ;
      //Om inte Paket produkt finns i LO rad then hämta in paketnr som matchar.
      if Length(Prefix) = 0 then
       Prefix  := dmsSystem.PkgNoToSuppCodeImportedPkg(PkgNo, L_SupplierNo, L_SupplierNo, -1, SupplierNo, ProductNo) ;
...
begin
 ...
 Begin
  cds_LOListHH.Insert ;
  cds_LOListHHSSPNo.AsInteger := cdsSawmillLoadOrdersSupplierShipPlanObjectNo.AsInteger ;
...
begin
 With dmcOrder do
 Begin
  acAddLOnoToSendList.Enabled := (cdsSawmillLoadOrders.Active) and (cdsSawmillLoadOrders.RecordCount > 0) ;
 End ;
end;
...
begin
 With dmcOrder do
 Begin
  acAddMarkedLONoToSendList.Enabled := (cdsSawmillLoadOrders.Active) and (cdsSawmillLoadOrders.RecordCount > 0) ;
 End ;
end;
...
begin
 With dmcOrder do
 Begin
  acSendLoadOrderNoToHH.Enabled := (cds_LOListHH.Active) and (cds_LOListHH.RecordCount > 0) ;
 End ;
end;
...
begin
 With dmcOrder do
 Begin
  acRemoveLOHHRow.Enabled := (cds_LOListHH.Active) and (cds_LOListHH.RecordCount > 0) ;
 End ;
end;
...
begin
 With dmcOrder do
 Begin
  acRemoveALLLOHHRows.Enabled := (cds_LOListHH.Active) and (cds_LOListHH.RecordCount > 0) ;
 End ;
end;

procedure TfrmLoadOrder.acDeleteALLLoad_ImpExecute(Sender: TObject);
begin
 if MessageDlg('Vill du ta alla rader?',  mtConfirmation, [mbYes, mbNo,mbCancel], 0) = mrYes then
 With dmsSystem do
 Begin
...
begin
 With dmsSystem do
 Begin
  acDeleteLoad_Imp.Enabled  := (cds_Load_Imp.Active) and (cds_Load_Imp.RecordCount > 0) ;
 End ;
end;
...
begin
 With dmsSystem do
 Begin
  acDeleteALLLoad_Imp.Enabled  := (cds_Load_Imp.Active) and (cds_Load_Imp.RecordCount > 0) ;
 End ;
end;
...
//        grdLoads.DataSource.DataSet.Bookmark := LoadBM;
 ...
 End ;//if..
        ...
        *)
procedure TfrmLoadOrder.SetCheckComboBoxes_Where_PktNrLevKod_Required(const VerkNo : Integer)  ;
Begin
 With dmcOrder do
 Begin
...
//   ccbIMP.Properties.Items.Add.Description  := sq_PC2.FieldByName('ProductCategoryName').AsString ;
  ...
  Begin
   ccVarugrupp.Properties.Items.AddCheckItem(sq_varugrupp.FieldByName('varugruppnamn').AsString, sq_varugrupp.FieldByName('varugruppno').AsString) ;
   sq_varugrupp.Next ;
  End ;
  sq_varugrupp.Close ;
